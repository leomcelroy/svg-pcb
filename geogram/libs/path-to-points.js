import { getPathData } from './get-path-data.js';
...
export function flattenPath( d /* svg */, options = {}) {
  const pathEl = document.createElementNS("http://www.w3.org/2000/svg", 'path'); //Create a path in SVG's namespace
  pathEl.setAttribute("d", d); //Set path's data

  const { maxError = 0.1 } = options;
  const paths = [];
  const xf = ([x, y]) => [x, y];
  const pathData = getPathData(pathEl, { normalize: true })
  let cur = null
  let closePoint = null
