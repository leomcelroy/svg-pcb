...
const VEML6040 = {"GND":{"shape":"M -0.016,0.006L 0.019,0.006L 0.019,-0.006L -0.016,-0.006L -0.016,0.006","pos":[-0.039,0.014],"layers":["F.Cu"],"index":1},"SDA":{"shape":"M -0.016,0.006L 0.019,0.006L 0.019,-0.006L -0.016,-0.006L -0.016,0.006","pos":[-0.039,-0.014],"layers":["F.Cu"],"index":2},"SCL":{"shape":"M -0.023,0.006L 0.016,0.006L 0.016,-0.006L -0.023,-0.006L -0.023,0.006","pos":[0.039,-0.014],"layers":["F.Cu"],"index":3},"VDD":{"shape":"M -0.019,0.006L 0.016,0.006L 0.016,-0.006L -0.019,-0.006L -0.019,0.006","pos":[0.039,0.014],"layers":["F.Cu"],"index":4}};
...
const height = .73 // board height
const x = 1 // x origin
const y = 1 // y origin
const zt = 0 // top z
const zb = -0.06 // bottom z
const w = .015 // wire width
const w1 = .012 // thinner  wire width
...
/* -- ADD_COMPONENTS -- */
let IC1 = board.add(ATtiny412, {translate: pt(x+.6, y+.47), name: 'IC1\nt412'});
let C1 = board.add(C_1206, {translate: pt(IC1.posX, IC1.padY("VCC")+.08), name: 'C1 1uF'});
let J1 = board.add(header_FTDI, {translate: pt(x+.23, IC1.posY-.1), rotate: 180, name: 'J1 serial\n5V power\n3.3V logic'});
let IC2 = board.add(regulator_SOT23, {translate: pt(IC1.posX-.23, C1.posY-.02), rotate: -90, name: 'IC2\n3.3V'});
let J2 = board.add(header_UPDI, {translate: pt(IC1.posX, y+.23), rotate: 90, name: 'J2\nUPDI'});
let IC3 = board.add(VEML6040, {translate: pt(x+width-.05, J2.posY+.03), rotate: 90, name: 'IC3'});
let R1 = board.add(R_1206, {translate: pt(IC3.posX-.12, IC3.posY-.15), name: 'R1 0'});
let R2 = board.add(R_1206, {translate: pt(J2.padX("GND")-.115, J2.posY), rotate: 90, name: 'R2\n4.99k'});
let R3 = board.add(R_1206, {translate: pt(J2.posX+.15, J2.posY+.03), rotate: 90, name: 'R3\n4.99k'});
...
board.wire(path(J1.pad("Tx"),
                ...
                pt(J1.posX+.1, IC1.padY("PA7")),
                IC1.pad("PA7")), w);

board.wire(path(J1.pad("Rx"),
                pt(J1.posX+.1, J1.padY("Rx")),
                pt(J1.posX+.1, IC1.padY("PA6")),
                IC1.pad("PA6")), w);
...
board.wire(path(J1.pad("VCC"),
                pt(J1.posX-.13, J1.padY("VCC")),
                pt(J1.posX-.13, J1.padY("RTS")+.05),
                pt(IC2.padX("in"), J1.padY("RTS")+.05)), w);

board.wire(path(J1.pad("GND"),
                pt(J1.posX+.09, J1.padY("GND")),
                pt(J1.posX+.09, J1.padY("VCC")+.05),
                pt(J1.posX-.1, J1.padY("VCC")+.05),
                pt(J1.posX-.1, J1.padY("Rx")+.05),
                pt(IC2.posX, J1.padY("Rx")+.05),
                IC2.pad("gnd")), w);

board.wire(path(IC2.pad("gnd"),
                pt(IC2.posX, IC2.padY("out")+.05),
                pt(IC1.posX, IC2.padY("out")+.05),
...
board.wire(path(J2.pad("GND"),
                pt(J2.padX("GND"), R1.posY),
                R1.pad("1")), w);

board.wire(path(R1.pad("2"),
                pt(R1.padX("2"), IC3.padY("GND")),
                IC3.pad("GND")), w1);

board.wire(path(pt(J2.padX("GND")-.05, R2.padY("1")),
                R2.pad("1")), w);

board.wire(path(pt(R2.posX-.07, R2.padY("2")),
                R2.pad("2")), w);

board.wire(path(IC1.pad("PA1"),
                pt(R2.padX("2")-.07, IC1.padY("PA1")),
                pt(R2.padX("2")-.07, R1.posY-.09),
                pt(IC3.padX("SDA"), R1.posY-.09)), w);

board.wire(path(pt(IC3.padX("SDA"), R1.posY-.09),
                IC3.pad("SDA")), w1);

board.wire(path(R3.pad("2"),
                pt(R3.posX, IC1.padY("PA2"))), w);

board.wire(path(R3.pad("1"),
                pt(R1.posX, R3.padY("1"))), w);

board.wire(path(IC3.pad("VDD"),
                pt(R1.posX, IC3.padY("VDD")),
                pt(R1.posX, R3.padY("1"))), w1);

board.wire(path(pt(R1.posX, R3.padY("1")),
                pt(R1.posX, R1.posY-.06),
                pt(J2.padX("GND")-.05, R1.posY-.06),
                pt(J2.padX("GND")-.05, IC1.padY("PA1")-.05),
                pt(IC1.posX-.03, IC1.padY("PA1")-.05),
