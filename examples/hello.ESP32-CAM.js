const header_FTDI = {"GND":{"shape":"M 0.05,0.025L -0.05,0.025L -0.0509,0.025L -0.0517,0.025L -0.0535,0.0248L -0.0552,0.0244L -0.056,0.0243L -0.0578,0.0237L -0.0593,0.0232L -0.0602,0.0229L -0.061,0.0225L -0.0618,0.0221L -0.0625,0.0216L -0.0633,0.0212L -0.064,0.0208L -0.0654,0.0197L -0.0661,0.0191L -0.0668,0.0186L -0.0686,0.0168L -0.0691,0.0161L -0.0697,0.0154L -0.0708,0.014L -0.0712,0.0133L -0.0716,0.0125L -0.0721,0.0118L -0.0725,0.011L -0.0729,0.0102L -0.0732,0.0093L -0.0737,0.0078L -0.0743,0.006L -0.0744,0.0052L -0.0748,0.0035L -0.075,0.0017L -0.075,0.0009L -0.075,0L -0.075,-0.0009L -0.075,-0.0017L -0.0748,-0.0035L -0.0744,-0.0052L -0.0743,-0.006L -0.0737,-0.0078L -0.0732,-0.0093L -0.0729,-0.0102L -0.0725,-0.011L -0.0721,-0.0118L -0.0716,-0.0125L -0.0712,-0.0133L -0.0708,-0.014L -0.0697,-0.0154L -0.0691,-0.0161L -0.0686,-0.0168L -0.0668,-0.0186L -0.0661,-0.0191L -0.0654,-0.0197L -0.064,-0.0208L -0.0633,-0.0212L -0.0625,-0.0216L -0.0618,-0.0221L -0.061,-0.0225L -0.0602,-0.0229L -0.0593,-0.0232L -0.0578,-0.0237L -0.056,-0.0243L -0.0552,-0.0244L -0.0535,-0.0248L -0.0517,-0.025L 0.05,-0.025L 0.05,0.025","pos":[0,0.25],"layers":["F.Cu"],"index":1},"CTS":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,0.15],"layers":["F.Cu"],"index":2},"VCC":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,0.05],"layers":["F.Cu"],"index":3},"Tx":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,-0.05],"layers":["F.Cu"],"index":4},"Rx":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,-0.15],"layers":["F.Cu"],"index":5},"RTS":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,-0.25],"layers":["F.Cu"],"index":6}};
const ESP32_CAM = {"+5V":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[-0.385,0.35],"layers":["F.Cu"],"index":1},"GND1":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[-0.515,0.25],"layers":["F.Cu"],"index":2},"GND2":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[0.385,-0.35],"layers":["F.Cu"],"index":9},"GND3":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[0.385,0.05],"layers":["F.Cu"],"index":13},"IO12":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[-0.385,0.15],"layers":["F.Cu"],"index":3},"IO13":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[-0.515,0.05],"layers":["F.Cu"],"index":4},"IO15":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[-0.385,-0.05],"layers":["F.Cu"],"index":5},"IO14":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[-0.515,-0.15],"layers":["F.Cu"],"index":6},"IO2":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[-0.385,-0.25],"layers":["F.Cu"],"index":7},"IO4":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[-0.515,-0.35],"layers":["F.Cu"],"index":8},"U0T":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[0.515,-0.25],"layers":["F.Cu"],"index":10},"U0R":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[0.385,-0.15],"layers":["F.Cu"],"index":11},"VCC":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[0.515,-0.05],"layers":["F.Cu"],"index":12},"IO0":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[0.515,0.15],"layers":["F.Cu"],"index":14},"IO16":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[0.385,0.25],"layers":["F.Cu"],"index":15},"3V3":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[0.515,0.35],"layers":["F.Cu"],"index":16}};
const slide_switch = {"1":{"shape":"M -0.0195,0.0235L 0.0195,0.0235L 0.0195,-0.0235L -0.0195,-0.0235L -0.0195,0.0235","pos":[-0.098,0.1],"layers":["F.Cu"],"index":1},"2":{"shape":"M -0.0195,0.0235L 0.0195,0.0235L 0.0195,-0.0235L -0.0195,-0.0235L -0.0195,0.0235","pos":[0,0.1],"layers":["F.Cu"],"index":2},"3":{"shape":"M -0.0195,0.0235L 0.0195,0.0235L 0.0195,-0.0235L -0.0195,-0.0235L -0.0195,0.0235","pos":[0.098,0.1],"layers":["F.Cu"],"index":3}};


// commands:
//  - SHIFT+ENTER: render
//  - drap & drop file: kicad import


// constants
const width = 1.1 // board width
const height = 1.32 // board height
const x = 1 // x origin
const y = 1 // y origin
const zt = 0 // top z
const zb = -0.06 // bottom z
const w = .015 // wire width
const mask = .004 // solder mask size
const border = 0.05 // rendering border


/* -- DECLARE_PCB -- */
let board = new PCB();

let interior = geo.translate(geo.rectangle(width, height), [x+width/2, y+height/2]);


/* -- ADD_COMPONENTS -- */
let J1 = board.add(header_FTDI, {translate: pt(x+width-.22, y+height/2), rotate: 0, name: 'J1 FTDI\n3.3V'});
let ESP = board.add(ESP32_CAM, {translate: pt(x+.4, y+.74), rotate: -90, name: 'ESP32-CAM'});
let S1 = board.add(slide_switch, {translate: pt(ESP.padX("IO0"), ESP.padY("IO0")-.19), name: 'prog run'});

board.addShape("interior", interior);


/* -- ADD_WIRES -- */
board.wire([ESP.pad("+5V"),
            pt(ESP.padX("+5V"), J1.padY("VCC")),
            J1.pad("VCC")], w);

board.wire([ESP.pad("GND1"),
            pt(J1.padX("GND"), ESP.padY("GND1")),
            J1.pad("GND")], w);

board.wire([ESP.pad("GND2"),
            pt(ESP.padX("GND2"), J1.padY("VCC")),
            pt(ESP.padX("GND1"), J1.padY("VCC")),
            ESP.pad("GND1")], w);

board.wire([ESP.pad("U0T"),
            pt(ESP.padX("U0T"), J1.padY("Rx")),
            J1.pad("Rx")], w);

board.wire([ESP.pad("U0R"),
            pt(ESP.padX("U0R"), J1.padY("Rx")-.05),
            pt(J1.padX("Tx")+.1, J1.padY("Rx")-.05),
            pt(J1.padX("Tx")+.1, J1.padY("Tx")),
            J1.pad("Tx")], w);

board.wire([S1.pad("1"),
            pt(ESP.padX("GND2"), S1.padY("1")),
            ESP.pad("GND2")], w);

board.wire([S1.pad("1"),
            pt(ESP.padX("GND3"), S1.padY("1")),
            ESP.pad("GND3")], w);

board.wire([S1.pad("2"),
            ESP.pad("IO0")], w);


// rendering
renderPCB({
  pcb: board,
  layerColors: {
    "interior": "#002d00ff",
    "B.Cu": "#ff4c007f",
    "F.Cu": "#ff8c00cc",
    "drill": "#ff3300e5",
    "padLabels": "#ffff99e5",
    "componentLabels": "#00e5e5e5",
  },
  limits: {
    x: [x-border, x+width+border],
    y: [y-border, y+height+border]
  },
  mm_per_unit: 25.4
})
