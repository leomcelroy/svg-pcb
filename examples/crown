...
const height = 1.36// board height
const x = 1 // x origin
const y = 1 // y origin
const zt = 0 // top z
...
/* -- ADD_COMPONENTS -- */
//let LED2 = board.add(LED_3014_1100, { translate: pt(1.41, 1.45), rotate: 90, name: "LED_3014_1100" })
let button = board.add(button_6mm, { translate: pt(1.29, 1.74), rotate: 90, name: "button_6mm" })
let IC1 = board.add(SAMD11C, {translate: pt(x+0.83, y+0.72), name: 'IC1\nD11'});
let J1 = board.add(header_SWD, {translate: pt(IC1.posX, IC1.padY("CLK")-0.24), rotate: 90, name: 'J1 SWD'});
let J2 = board.add(USB_A_plug, {translate: pt(IC1.posX, y+height-.29), rotate: 90, name: 'J2\nUSB'});
let IC2 = board.add(regulator_SOT23, {translate: pt(IC1.padX("A05")-0.58, IC1.padY("CLK")-0.23), rotate: -90, name: 'IC2\n3.3V'});
let C1 = board.add(C_1206, {translate: pt(IC2.posX, IC2.posY-0.2), rotate: 90, name: 'C1\n1uF'});
let R1 = board.add(R_1206, {translate: pt(J1.padX("VCC")+0.11, J1.posY), rotate: 90, name: 'R1\n0'});
let R2 = board.add(R_1206, {translate: pt(IC1.padX("A08")-0.17, IC1.padY("A08")-0.43), rotate: 90, name: 'R2\n1k'});
let LED1 = board.add(LED_1206, {translate: pt(R2.posX-0.12, R2.posY), rotate: 90, name: 'LED1'});
...
board.wire(path(J2.pad("D-"),
                pt(IC1.posX-0.015, J2.padY("D-")-0.075),
                pt(IC1.posX-0.015, IC1.padY("24-")),
                IC1.pad("24-"),), 0.012);

board.wire(path(J2.pad("D+"),
                pt(IC1.posX+0.02, J2.padY("D+")-0.075),
                pt(IC1.posX+0.02, IC1.padY("25+")),
                IC1.pad("25+"),), 0.012);

board.wire(path(J2.pad("GND"),
                pt(J2.padX("GND"), J2.padY("GND")-.11),
                pt(IC1.posX+0.055, J2.padY("GND")-0.11),
                pt(IC1.posX+0.055, IC1.padY("GND")),
                IC1.pad("GND"),), 0.012);

board.wire(path(IC2.pad("in"),
                pt(IC2.padX("in"), J2.padY("5V")-0.08),
                pt(J2.padX("5V"), J2.padY("5V")-.08),
                J2.pad("5V"),), w);

board.wire(path(IC2.pad("gnd"),
                pt(IC2.posX, J2.padY("GND")-.11),
                pt(IC1.posX-0.05, J2.padY("GND")-0.11),
                pt(IC1.posX-0.05, IC1.padY("CLK")),
                pt(J1.padX("GND2"), IC1.padY("CLK")),
                J1.pad("GND2"),), 0.012);

board.wire(path(C1.pad("1"),
                pt(C1.posX+0.06, C1.padY("1")),
...
board.wire(path(J1.pad("VCC"),
                ...
                pt(R1.posX+0.07, J1.posY),
                pt(R1.posX+0.07, IC1.padY("VDD")),
                IC1.pad("VDD"),), w);

board.wire(path(C1.pad("2"),
                pt(C1.posX-.06, C1.padY("2")),
                pt(C1.posX-0.06, J1.padY("DIO")-0.233),
                pt(R1.posX+0.21, J1.padY("DIO")-0.233),
                pt(R1.posX+0.21, J2.padY("GND")-0.11),
                pt(J2.padX("GND"), J2.padY("GND")-.11),
                J2.pad("GND"),), w);

board.wire(path(C1.pad("1"),
                pt(C1.posX+.06, C1.padY("1")),
                pt(C1.posX+0.06, J1.padY("DIO")-0.103),
                pt(R1.posX+0.07, J1.padY("DIO")-0.103),
                pt(R1.posX+0.07, IC1.padY("VDD")),
                IC1.pad("VDD"),), w);

board.wire(path(R2.pad("2"),
                pt(R2.posX, IC1.padY("A15")),
                IC1.pad("A15"),), w);
...
board.wire(path(button.pad("L1"),
                pt(IC2.posX, button.padY("L1")),
                IC2.pad("gnd"),), w);

board.wire(path(button.pad("R2"),
  pt(button.padX("R2")+0.13, 1.86),
  pt(1.51, 1.72),
  IC1.pad("A14"),), w);
