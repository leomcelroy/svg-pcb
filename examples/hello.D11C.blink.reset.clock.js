...
/* -- ADD_COMPONENTS -- */
let IC1 = board.add(SAMD11C, {translate: pt(x+.62, y+.55), name: 'IC1\nD11'});
...
let IC2 = board.add(regulator_SOT23, {translate: pt(IC1.padX("A05")-.42, IC1.padY("CLK")), rotate: -90, name: 'IC2\n3.3V'});
...
let R2 = board.add(R_1206, {translate: pt(IC1.padX("A08")-.15, IC1.padY("A08")-.03), rotate: 90, name: 'R2\n1k'});
let LED1 = board.add(LED_1206, {translate: pt(R2.posX-.1, R2.posY), rotate: 90, name: 'LED1'});
let R3 = board.add(R_1206, {translate: pt(J1.posX-.23, J1.posY), name: 'R3 10k'});
let R4 = board.add(R_1206, {translate: pt(R3.padX("1"), R3.posY+.15), rotate: 90, name: 'R4\n10k'});
let C2 = board.add(C_1206, {translate: pt(R4.posX-.12, R4.posY), rotate: 90, name: 'C2\n.1uF'});
...
board.wire(path(IC1.pad("CLK"),
                pt(R3.padX("2"), IC1.padY("CLK")),
                R3.pad("2")), w);

board.wire(path(J1.pad("RST"),
                pt(R3.posX, J1.padY("RST")),
                pt(R3.posX, IC1.padY("RST")),
                IC1.pad("RST")), w);

board.wire(path(R4.pad("2"),
                pt(R3.posX, R4.padY("2"))), w);

board.wire(path(C1.pad("2"),
                pt(C2.posX, C1.padY("2"))), w);

board.wire(path(C2.pad("2"),
                R4.pad("2")), w);

board.wire(path(C1.pad("1"),
                pt((C2.posX+R4.posX)/2, C1.padY("1")),
                pt((C2.posX+R4.posX)/2, C2.posY),
                pt(IC2.padX("out"), C2.posY),
                IC2.pad("out")), w);

board.wire(path(C1.pad("1"),
                pt((C2.posX+R4.posX)/2, C1.padY("1")),
                pt((C2.posX+R4.posX)/2, J1.padY("DIO")-.07),
                pt(R1.posX+.06, J1.padY("DIO")-.07),
                pt(R1.posX+.06, IC1.padY("VDD")),
                IC1.pad("VDD")), w);

board.wire(path(R3.pad("1"),
                pt((C2.posX+R4.posX)/2, R3.posY)), w);

board.wire(path(R4.pad("1"),
                pt((C2.posX+R4.posX)/2, R4.padY("1"))), w);
