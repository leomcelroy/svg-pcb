const ATtiny44_SOICN = footprint({"VCC":{"shape":"M 0.035,0.015L -0.035,0.015L -0.0355,0.015L -0.036,0.015L -0.0371,0.0149L -0.0386,0.0146L -0.0391,0.0144L -0.0396,0.0142L -0.0401,0.0141L -0.0406,0.0139L -0.0416,0.0135L -0.0425,0.013L -0.0429,0.0127L -0.0434,0.0125L -0.0442,0.0118L -0.0447,0.0115L -0.045,0.0112L -0.0454,0.0108L -0.0458,0.0104L -0.0461,0.0101L -0.0465,0.0096L -0.0474,0.0084L -0.0477,0.008L -0.048,0.0075L -0.0482,0.0071L -0.0485,0.0066L -0.0487,0.0061L -0.0489,0.0056L -0.0491,0.0051L -0.0493,0.0047L -0.0494,0.0041L -0.0495,0.0036L -0.0497,0.0031L -0.0498,0.0026L -0.05,0.0005L -0.05,-0.0005L -0.0498,-0.0026L -0.0497,-0.0031L -0.0495,-0.0036L -0.0494,-0.0041L -0.0493,-0.0047L -0.0491,-0.0051L -0.0489,-0.0056L -0.0487,-0.0061L -0.0485,-0.0066L -0.0482,-0.0071L -0.048,-0.0075L -0.0477,-0.008L -0.0474,-0.0084L -0.0465,-0.0096L -0.0461,-0.0101L -0.0458,-0.0104L -0.0454,-0.0108L -0.045,-0.0112L -0.0447,-0.0115L -0.0442,-0.0118L -0.0434,-0.0125L -0.0429,-0.0127L -0.0425,-0.013L -0.0416,-0.0135L -0.0406,-0.0139L -0.0401,-0.0141L -0.0396,-0.0142L -0.0391,-0.0144L -0.0386,-0.0146L -0.0371,-0.0149L -0.036,-0.015L 0.035,-0.015L 0.035,0.015","pos":[-0.12,0.15],"layers":["F.Cu","F.Mask"],"index":1},"PB0":{"shape":"M -0.035,0.015L 0.035,0.015L 0.035,-0.015L -0.035,-0.015L -0.035,0.015","pos":[-0.12,0.1],"layers":["F.Cu","F.Mask"],"index":2},"PB1":{"shape":"M -0.035,0.015L 0.035,0.015L 0.035,-0.015L -0.035,-0.015L -0.035,0.015","pos":[-0.12,0.05],"layers":["F.Cu","F.Mask"],"index":3},"PB3":{"shape":"M -0.035,0.015L 0.035,0.015L 0.035,-0.015L -0.035,-0.015L -0.035,0.015","pos":[-0.12,0],"layers":["F.Cu","F.Mask"],"index":4},"PB2":{"shape":"M -0.035,0.015L 0.035,0.015L 0.035,-0.015L -0.035,-0.015L -0.035,0.015","pos":[-0.12,-0.05],"layers":["F.Cu","F.Mask"],"index":5},"PA7":{"shape":"M -0.035,0.015L 0.035,0.015L 0.035,-0.015L -0.035,-0.015L -0.035,0.015","pos":[-0.12,-0.1],"layers":["F.Cu","F.Mask"],"index":6},"PA6":{"shape":"M -0.035,0.015L 0.035,0.015L 0.035,-0.015L -0.035,-0.015L -0.035,0.015","pos":[-0.12,-0.15],"layers":["F.Cu","F.Mask"],"index":7},"PA5":{"shape":"M -0.035,0.015L 0.035,0.015L 0.035,-0.015L -0.035,-0.015L -0.035,0.015","pos":[0.12,-0.15],"layers":["F.Cu","F.Mask"],"index":8},"PA4":{"shape":"M -0.035,0.015L 0.035,0.015L 0.035,-0.015L -0.035,-0.015L -0.035,0.015","pos":[0.12,-0.1],"layers":["F.Cu","F.Mask"],"index":9},"PA3":{"shape":"M -0.035,0.015L 0.035,0.015L 0.035,-0.015L -0.035,-0.015L -0.035,0.015","pos":[0.12,-0.05],"layers":["F.Cu","F.Mask"],"index":10},"PA2":{"shape":"M -0.035,0.015L 0.035,0.015L 0.035,-0.015L -0.035,-0.015L -0.035,0.015","pos":[0.12,0],"layers":["F.Cu","F.Mask"],"index":11},"PA1":{"shape":"M -0.035,0.015L 0.035,0.015L 0.035,-0.015L -0.035,-0.015L -0.035,0.015","pos":[0.12,0.05],"layers":["F.Cu","F.Mask"],"index":12},"PA0":{"shape":"M -0.035,0.015L 0.035,0.015L 0.035,-0.015L -0.035,-0.015L -0.035,0.015","pos":[0.12,0.1],"layers":["F.Cu","F.Mask"],"index":13},"GND":{"shape":"M -0.035,0.015L 0.035,0.015L 0.035,-0.015L -0.035,-0.015L -0.035,0.015","pos":[0.12,0.15],"layers":["F.Cu","F.Mask"],"index":14}});
const header_ISP = footprint({"MISO":{"shape":"M -0.05,-0.025L 0.0517,-0.025L 0.0535,-0.0248L 0.0552,-0.0244L 0.056,-0.0243L 0.0578,-0.0237L 0.0593,-0.0232L 0.0602,-0.0229L 0.061,-0.0225L 0.0618,-0.0221L 0.0625,-0.0216L 0.0633,-0.0212L 0.064,-0.0208L 0.0654,-0.0197L 0.0661,-0.0191L 0.0668,-0.0186L 0.0686,-0.0168L 0.0691,-0.0161L 0.0697,-0.0154L 0.0708,-0.014L 0.0712,-0.0133L 0.0716,-0.0125L 0.0721,-0.0118L 0.0725,-0.011L 0.0729,-0.0102L 0.0732,-0.0093L 0.0737,-0.0078L 0.0743,-0.006L 0.0744,-0.0052L 0.0748,-0.0035L 0.075,-0.0017L 0.075,-0.0009L 0.075,0L 0.075,0.0009L 0.075,0.0017L 0.0748,0.0035L 0.0744,0.0052L 0.0743,0.006L 0.0737,0.0078L 0.0732,0.0093L 0.0729,0.0102L 0.0725,0.011L 0.0721,0.0118L 0.0716,0.0125L 0.0712,0.0133L 0.0708,0.014L 0.0697,0.0154L 0.0691,0.0161L 0.0686,0.0168L 0.0668,0.0186L 0.0661,0.0191L 0.0654,0.0197L 0.064,0.0208L 0.0633,0.0212L 0.0625,0.0216L 0.0618,0.0221L 0.061,0.0225L 0.0602,0.0229L 0.0593,0.0232L 0.0578,0.0237L 0.056,0.0243L 0.0552,0.0244L 0.0535,0.0248L 0.0517,0.025L 0.0509,0.025L 0.05,0.025L -0.05,0.025L -0.05,-0.025","pos":[0.107,-0.1],"layers":["F.Cu","F.Mask"],"index":1},"V":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[-0.107,-0.1],"layers":["F.Cu","F.Mask"],"index":2},"SCK":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0.107,0],"layers":["F.Cu","F.Mask"],"index":3},"MOSI":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[-0.107,0],"layers":["F.Cu","F.Mask"],"index":4},"RST":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0.107,0.1],"layers":["F.Cu","F.Mask"],"index":5},"GND":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[-0.107,0.1],"layers":["F.Cu","F.Mask"],"index":6}});
const header_FTDI = footprint({"GND":{"shape":"M 0.05,0.025L -0.05,0.025L -0.0509,0.025L -0.0517,0.025L -0.0535,0.0248L -0.0552,0.0244L -0.056,0.0243L -0.0578,0.0237L -0.0593,0.0232L -0.0602,0.0229L -0.061,0.0225L -0.0618,0.0221L -0.0625,0.0216L -0.0633,0.0212L -0.064,0.0208L -0.0654,0.0197L -0.0661,0.0191L -0.0668,0.0186L -0.0686,0.0168L -0.0691,0.0161L -0.0697,0.0154L -0.0708,0.014L -0.0712,0.0133L -0.0716,0.0125L -0.0721,0.0118L -0.0725,0.011L -0.0729,0.0102L -0.0732,0.0093L -0.0737,0.0078L -0.0743,0.006L -0.0744,0.0052L -0.0748,0.0035L -0.075,0.0017L -0.075,0.0009L -0.075,0L -0.075,-0.0009L -0.075,-0.0017L -0.0748,-0.0035L -0.0744,-0.0052L -0.0743,-0.006L -0.0737,-0.0078L -0.0732,-0.0093L -0.0729,-0.0102L -0.0725,-0.011L -0.0721,-0.0118L -0.0716,-0.0125L -0.0712,-0.0133L -0.0708,-0.014L -0.0697,-0.0154L -0.0691,-0.0161L -0.0686,-0.0168L -0.0668,-0.0186L -0.0661,-0.0191L -0.0654,-0.0197L -0.064,-0.0208L -0.0633,-0.0212L -0.0625,-0.0216L -0.0618,-0.0221L -0.061,-0.0225L -0.0602,-0.0229L -0.0593,-0.0232L -0.0578,-0.0237L -0.056,-0.0243L -0.0552,-0.0244L -0.0535,-0.0248L -0.0517,-0.025L 0.05,-0.025L 0.05,0.025","pos":[0,0.25],"layers":["F.Cu","F.Mask"],"index":1},"CTS":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,0.15],"layers":["F.Cu","F.Mask"],"index":2},"VCC":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,0.05],"layers":["F.Cu","F.Mask"],"index":3},"Tx":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,-0.05],"layers":["F.Cu","F.Mask"],"index":4},"Rx":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,-0.15],"layers":["F.Cu","F.Mask"],"index":5},"RTS":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,-0.25],"layers":["F.Cu","F.Mask"],"index":6}});
const R_1206 = footprint({"1":{"shape":"M -0.032,0.034L 0.032,0.034L 0.032,-0.034L -0.032,-0.034L -0.032,0.034","pos":[-0.06,0],"layers":["F.Cu","F.Mask"],"index":1},"2":{"shape":"M -0.032,0.034L 0.032,0.034L 0.032,-0.034L -0.032,-0.034L -0.032,0.034","pos":[0.06,0],"layers":["F.Cu","F.Mask"],"index":2}});
const regulator_SOT23 = footprint({"out":{"shape":"M -0.02,0.012L 0.02,0.012L 0.02,-0.012L -0.02,-0.012L -0.02,0.012","pos":[-0.045,0.0375],"layers":["F.Cu","F.Mask"],"index":1},"in":{"shape":"M -0.02,0.012L 0.02,0.012L 0.02,-0.012L -0.02,-0.012L -0.02,0.012","pos":[-0.045,-0.0375],"layers":["F.Cu","F.Mask"],"index":2},"gnd":{"shape":"M -0.02,0.012L 0.02,0.012L 0.02,-0.012L -0.02,-0.012L -0.02,0.012","pos":[0.045,0],"layers":["F.Cu","F.Mask"],"index":3}});
const C_1206 = footprint({"1":{"shape":"M -0.032,0.034L 0.032,0.034L 0.032,-0.034L -0.032,-0.034L -0.032,0.034","pos":[-0.06,0],"layers":["F.Cu","F.Mask"],"index":1},"2":{"shape":"M -0.032,0.034L 0.032,0.034L 0.032,-0.034L -0.032,-0.034L -0.032,0.034","pos":[0.06,0],"layers":["F.Cu","F.Mask"],"index":2}});
const microSD = footprint({"NC1":{"shape":"M -0.0138,0.034L 0.0138,0.034L 0.0138,-0.034L -0.0138,-0.034L -0.0138,0.034","pos":[0.1261,-0.304],"layers":["F.Cu","F.Mask"],"index":1},"NC2":{"shape":"M -0.0138,0.034L 0.0138,0.034L 0.0138,-0.034L -0.0138,-0.034L -0.0138,0.034","pos":[-0.177,-0.304],"layers":["F.Cu","F.Mask"],"index":8},"NC3":{"shape":"M -0.021,0.029L 0.021,0.029L 0.021,-0.029L -0.021,-0.029L -0.021,0.029","pos":[-0.228,-0.299],"layers":["F.Cu","F.Mask"],"index":0},"NC4":{"shape":"M -0.029,0.029L 0.029,0.029L 0.029,-0.029L -0.029,-0.029L -0.029,0.029","pos":[0.222,-0.299],"layers":["F.Cu","F.Mask"],"index":0},"NC5":{"shape":"M -0.015,0.027L 0.015,0.027L 0.015,-0.027L -0.015,-0.027L -0.015,0.027","pos":[-0.232,-0.002],"layers":["F.Cu","F.Mask"],"index":0},"NC6":{"shape":"M -0.015,0.029L 0.015,0.029L 0.015,-0.029L -0.015,-0.029L -0.015,0.029","pos":[0.238,0.025],"layers":["F.Cu","F.Mask"],"index":0},"NC7":{"shape":"M -0.028,0.019L 0.028,0.019L 0.028,-0.019L -0.028,-0.019L -0.028,0.019","pos":[-0.221,0.059],"layers":["F.Cu","F.Mask"],"index":0},"NC8":{"shape":"M -0.019,0.03L 0.019,0.03L 0.019,-0.03L -0.019,-0.03L -0.019,0.03","pos":[0.222,0.121],"layers":["F.Cu","F.Mask"],"index":0},"SS":{"shape":"M -0.0138,0.034L 0.0138,0.034L 0.0138,-0.034L -0.0138,-0.034L -0.0138,0.034","pos":[0.0828,-0.304],"layers":["F.Cu","F.Mask"],"index":2},"MOSI":{"shape":"M -0.0138,0.034L 0.0138,0.034L 0.0138,-0.034L -0.0138,-0.034L -0.0138,0.034","pos":[0.0395,-0.304],"layers":["F.Cu","F.Mask"],"index":3},"VCC":{"shape":"M -0.0138,0.034L 0.0138,0.034L 0.0138,-0.034L -0.0138,-0.034L -0.0138,0.034","pos":[-0.0038,-0.304],"layers":["F.Cu","F.Mask"],"index":4},"SCK":{"shape":"M -0.0138,0.034L 0.0138,0.034L 0.0138,-0.034L -0.0138,-0.034L -0.0138,0.034","pos":[-0.0471,-0.304],"layers":["F.Cu","F.Mask"],"index":5},"GND":{"shape":"M -0.0138,0.034L 0.0138,0.034L 0.0138,-0.034L -0.0138,-0.034L -0.0138,0.034","pos":[-0.0904,-0.304],"layers":["F.Cu","F.Mask"],"index":6},"MISO":{"shape":"M -0.0138,0.034L 0.0138,0.034L 0.0138,-0.034L -0.0138,-0.034L -0.0138,0.034","pos":[-0.1337,-0.304],"layers":["F.Cu","F.Mask"],"index":7}});


// @version: v0.1.0
// commands:
//  - SHIFT+ENTER: render
//  - drap & drop file: kicad import


// constants
const width = 1.59 // board width
const height = .97 // board height
const x = 1 // x origin
const y = 1 // y origin
const zt = 0 // top z
const zb = -0.06 // bottom z
const w = .015 // wire width
const mask = .004 // solder mask size
const border = 0.05 // rendering border


/* -- DECLARE_PCB -- */
let board = new PCB();



/* -- ADD_COMPONENTS -- */
let IC1 = board.add(ATtiny44_SOICN, {translate: pt(x+.86, y+.59), label: 'IC1\nt44'});
let J1 = board.add(header_ISP, {translate: pt(IC1.posX+.05, IC1.padY("PA6")-.22), rotate: 90, label: 'J1\nISP'});
let J2 = board.add(header_FTDI, {translate: pt(x+width-.22, IC1.posY-.0), rotate: 0, label: 'J2 FTDI'});
let R1 = board.add(R_1206, {translate: pt(IC1.posX, IC1.padY("VCC")+.1), label: 'R1\n10k'});
let IC2 = board.add(regulator_SOT23, {translate: pt(J2.posX-.16, J2.padY("CTS")), rotate: 90, label: 'IC2\n3.3V'});
let C1 = board.add(C_1206, {translate: pt(IC2.posX-.12, IC2.posY), rotate: 90, label: 'C1\n1uF'});
let J3 = board.add(microSD, {translate: pt(IC1.posX-.42, IC1.posY-.1), rotate: -90, label: 'J3\nmicroSD'});

let interior = geo.path(path([1.000, 1.970],
                             [2.590, 1.970],
                             [2.590, 1.000],
                             [1.000, 1.000]));

board.addShape("interior", interior);


/* -- ADD_WIRES -- */
board.wire(path(IC1.pad("PA5"),
                [J1.padX("MISO"), IC1.padY("PA5")],
                J1.pad("MISO"),), w);

board.wire(path(IC1.pad("PA4"),
                [J1.padX("SCK"), IC1.padY("PA4")],
                J1.pad("SCK"),), w);

board.wire(path(IC1.pad("PA6"),
                [IC1.padX("PA6"), J1.posY+.02],
                [J1.padX("MOSI"), J1.posY+.02],
                J1.pad("MOSI"),), w);

board.wire(path(IC1.pad("PB3"),
                [J1.padX("RST"), IC1.padY("PB3")],
                J1.pad("RST"),), w);

board.wire(path(J2.pad("GND"),
                [IC1.padX("GND"), J2.padY("GND")],
                IC1.pad("GND"),), w);

board.wire(path(IC1.pad("PA2"),
                [IC1.padX("PA2")+.1, IC1.padY("PA2")],
                [IC1.padX("PA2")+.13, J2.padY("Tx")],
                J2.pad("Tx"),), w);

board.wire(path(IC1.pad("PA3"),
                [IC1.padX("PA3")+.07, IC1.padY("PA3")],
                [IC1.padX("PA3")+.12, J2.padY("Rx")],
                J2.pad("Rx"),), w);

board.wire(path(R1.pad("1"),
                [J1.padX("RST"), R1.posY],
                J1.pad("RST"),), w);

board.wire(path(IC1.pad("PA0"),
                [R1.padX("2"), IC1.padY("PA0")],
                R1.pad("2"),), w);

board.wire(path(R1.pad("2"),
                [R1.padX("2"), R1.posY+.06],
                [IC1.padX("VCC"), R1.posY+.06],
                IC1.pad("VCC"),), w);

board.wire(path(J2.pad("VCC"),
                [IC2.padX("in"), J2.padY("VCC")],
                IC2.pad("in"),), w);

board.wire(path(J2.pad("GND"),
                [IC2.padX("gnd"), J2.padY("GND")],
                IC2.pad("gnd"),), w);

board.wire(path(J1.pad("V"),
                [J1.padX("V"), J1.padY("V")-.08],
                [J2.posX+.08, J1.padY("V")-.08],
                [J2.posX+.08, J2.padY("Tx")+.05],
                [IC2.padX("out"), J2.padY("Tx")+.05],
                IC2.pad("out"),), w);

board.wire(path(C1.pad("1"),
                [C1.padX("1"), IC2.padY("out")],
                IC2.pad("out"),), w);

board.wire(path(J2.pad("GND"),
                [C1.padX("2"), J2.padY("GND")],
                C1.pad("2"),), w);

board.wire(path(IC1.pad("PA0"),
                [C1.padX("1"), IC1.padY("PA0")],
                C1.pad("1"),), w);

board.wire(path(J3.pad("SS"),
                [J3.padX("SS")+.07, J3.padY("SS")],
                [J3.padX("SS")+.07, J3.posY-.315],
                [IC1.padX("PA7")-.08, J3.posY-.315],
                [IC1.padX("PA7")-.08, IC1.padY("PA7")],
                IC1.pad("PA7"),), w);

board.wire(path(J3.pad("MOSI"),
                [J3.padX("MOSI")+.105, J3.padY("MOSI")],
                [J3.padX("MOSI")+.105, J3.posY-.28],
                [IC1.padX("PB2")-.115, J3.posY-.28],
                [IC1.padX("PB2")-.115, IC1.padY("PB2")],
                IC1.pad("PB2"),), w);

board.wire(path(J1.pad("V"),
                [J1.padX("V"), J1.padY("V")-.08],
                [J1.padX("GND")+.05, J1.padY("V")-.08],
                [J1.padX("GND")+.05, J1.posY-.02],
                [IC1.padX("PA6")+.01, J1.posY-.02],
                [IC1.padX("PA6")+.01, J1.padY("GND")],
                [J3.padX("VCC")-.07, J1.padY("GND")],
                [J3.padX("VCC")-.07, J3.padY("VCC")],
                J3.pad("VCC"),), w);

board.wire(path(J3.pad("SCK"),
                [J3.padX("SCK")+.105, J3.padY("SCK")],
                [J3.padX("SCK")+.105, J3.posY+.28],
                [IC1.padX("PB1")-.115, J3.posY+.28],
                [IC1.padX("PB1")-.115, IC1.padY("PB1")],
                IC1.pad("PB1"),), w);

board.wire(path(J1.pad("GND"),
                [J1.padX("GND"), J1.padY("GND")-.08],
                [J3.padX("GND")-.105, J1.padY("GND")-.08],
                [J3.padX("GND")-.105, J3.padY("GND")],
                J3.pad("GND"),), w);

board.wire(path(IC1.pad("GND"),
                [IC1.padX("GND"), R1.posY+.095],
                [J3.padX("GND")-.105, R1.posY+.095],
                [J3.padX("GND")-.105, J3.padY("GND")],
                J3.pad("GND"),), w);

board.wire(path(J3.pad("MISO"),
                [J3.padX("MISO")+.07, J3.padY("MISO")],
                [J3.padX("MISO")+.07, J3.posY+.315],
                [IC1.padX("PB0")-.08, J3.posY+.315],
                [IC1.padX("PB0")-.08, IC1.padY("PB0")],
                IC1.pad("PB0"),), w);


// rendering
renderPCB({
  pcb: board,
  layerColors: {
    "interior": "#002d00ff",
    "B.Cu": "#ff4c007f",
    "F.Cu": "#be7a27cc",
    "B.Mask": "#ff814bff",
    "F.Mask": "#ffa50aff",
    "padLabels": "#ffff99e5",
    "componentLabels": "#00e5e5e5",
  },
  limits: {
    x: [x-border, x+width+border],
    y: [y-border, y+height+border]
  },
  mm_per_unit: 25.4
})
