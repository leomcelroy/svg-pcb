const SAMD11C = {"A05":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[-0.11,0.15],"layers":["F.Cu"],"index":1},"A08":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[-0.11,0.1],"layers":["F.Cu"],"index":2},"A09":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[-0.11,0.05],"layers":["F.Cu"],"index":3},"A14":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[-0.11,0],"layers":["F.Cu"],"index":4},"A15":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[-0.11,-0.05],"layers":["F.Cu"],"index":5},"RST":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[-0.11,-0.1],"layers":["F.Cu"],"index":6},"CLK":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[-0.11,-0.15],"layers":["F.Cu"],"index":7},"DIO":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,-0.15],"layers":["F.Cu"],"index":8},"24-":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,-0.1],"layers":["F.Cu"],"index":9},"25+":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,-0.05],"layers":["F.Cu"],"index":10},"GND":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,0],"layers":["F.Cu"],"index":11},"VDD":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,0.05],"layers":["F.Cu"],"index":12},"A02":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,0.1],"layers":["F.Cu"],"index":13},"A04":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,0.15],"layers":["F.Cu"],"index":14}};
const header_SWD_4_1 = {"CLK":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[-0.107,0.05],"layers":["F.Cu"],"index":1},"DIO":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0.107,0.05],"layers":["F.Cu"],"index":2},"RST":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[-0.107,-0.05],"layers":["F.Cu"],"index":3},"GND":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0.107,-0.05],"layers":["F.Cu"],"index":4}};
const USB_A_plug = {"5V":{"shape":"M -0.04999999999999999,0.02L 0.242,0.02L 0.242,-0.02L -0.04999999999999999,-0.02L -0.04999999999999999,0.02","pos":[0,0.138],"layers":["F.Cu"],"index":1},"D-":{"shape":"M -0.05,0.02L 0.202,0.02L 0.202,-0.02L -0.05,-0.02L -0.05,0.02","pos":[0,0.039],"layers":["F.Cu"],"index":2},"D+":{"shape":"M -0.05,0.02L 0.202,0.02L 0.202,-0.02L -0.05,-0.02L -0.05,0.02","pos":[0,-0.039],"layers":["F.Cu"],"index":3},"GND":{"shape":"M -0.04999999999999999,0.02L 0.242,0.02L 0.242,-0.02L -0.04999999999999999,-0.02L -0.04999999999999999,0.02","pos":[0,-0.138],"layers":["F.Cu"],"index":4}};
const regulator_SOT23 = {"out":{"shape":"M -0.02,0.012L 0.02,0.012L 0.02,-0.012L -0.02,-0.012L -0.02,0.012","pos":[-0.045,0.0375],"layers":["F.Cu"],"index":1},"in":{"shape":"M -0.02,0.012L 0.02,0.012L 0.02,-0.012L -0.02,-0.012L -0.02,0.012","pos":[-0.045,-0.0375],"layers":["F.Cu"],"index":2},"gnd":{"shape":"M -0.02,0.012L 0.02,0.012L 0.02,-0.012L -0.02,-0.012L -0.02,0.012","pos":[0.045,0],"layers":["F.Cu"],"index":3}};
const C_1206 = {"1":{"shape":"M -0.032,0.034L 0.032,0.034L 0.032,-0.034L -0.032,-0.034L -0.032,0.034","pos":[-0.06,0],"layers":["F.Cu"],"index":1},"2":{"shape":"M -0.032,0.034L 0.032,0.034L 0.032,-0.034L -0.032,-0.034L -0.032,0.034","pos":[0.06,0],"layers":["F.Cu"],"index":2}};
const header_serial_reverse = {"GND":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,-0.25],"layers":["F.Cu"],"index":1},"CTS":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,-0.15],"layers":["F.Cu"],"index":2},"VCC":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,-0.05],"layers":["F.Cu"],"index":3},"Tx":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,0.05],"layers":["F.Cu"],"index":4},"Rx":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,0.15],"layers":["F.Cu"],"index":5},"RTS":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,0.25],"layers":["F.Cu"],"index":6}};
const R_1206 = {"1":{"shape":"M -0.032,0.034L 0.032,0.034L 0.032,-0.034L -0.032,-0.034L -0.032,0.034","pos":[-0.06,0],"layers":["F.Cu"],"index":1},"2":{"shape":"M -0.032,0.034L 0.032,0.034L 0.032,-0.034L -0.032,-0.034L -0.032,0.034","pos":[0.06,0],"layers":["F.Cu"],"index":2}};


// commands:
//  - SHIFT+ENTER: render
//  - drap & drop file: kicad import


// constants
const width = .78 // board width
const height = 1.41 // board height
const x = 1 // x origin
const y = 1 // y origin
const zt = 0 // top z
const zb = -0.06 // bottom z
const rv = 0.007 // via radius
const rp = 0.014 // pad radius
const w = .015 // wire width
const mask = .004 // solder mask size
const border = 0.05 // rendering border


/* -- DECLARE_PCB -- */
let board = new PCB();

let interior = geo.translate(geo.rectangle(width, height), [x+width/2, y+height/2]);


/* -- ADD_COMPONENTS -- */
let IC1 = board.add(SAMD11C, {translate: pt(x+.37, y+height-.63), name: 'IC1\nD11C'});
let J1 = board.add(header_SWD_4_1, {translate: pt(IC1.posX+.015, IC1.padY("CLK")-.14), name: 'J1 SWD'});
let J2 = board.add(USB_A_plug, {translate: pt(x+width/2, y+height-.29), rotate: 90, name: 'J2\nUSB'});
let IC2 = board.add(regulator_SOT23, {translate: pt(IC1.padX("A04")+.17, IC1.padY("A08")-.04), rotate: 90, name: 'IC2\n3.3V'});
let C1 = board.add(C_1206, {translate: pt(IC2.posX, IC2.posY-.2), rotate: 90, name: 'C1\n1uF'});
let J3 = board.add(header_serial_reverse, {translate: pt(IC1.posX+.02, y+.23), rotate: -90, name: 'J3 serial'});
let R1 = board.add(R_1206, {translate: pt(J1.posX-.29, J1.posY), rotate: 90, name: 'R1\n4.99k'});
let V1 = board.add(via(rv, rp), {translate: pt(IC1.padX("A14")-.1, IC1.padY("A14")), name: 'V1'});
let V2 = board.add(via(rv, rp), {translate: pt(J3.padX("CTS")-.025, J3.padY("CTS")-.11), name: 'V2'});
let V3 = board.add(via(rv, rp), {translate: pt(IC1.padX("A04")+.1, IC1.padY("A04")), name: 'V3'});
let V4 = board.add(via(rv, rp), {translate: pt(J3.padX("Tx")+.025, J3.padY("Tx")-.11), name: 'V4'});

geo.difference(interior, geo.rotate(geo.translate(geo.rectangle(1.05, 9.76), [0.475+J2.posX, 5.12+J2.posY]), 90.00000001, J2.pos));
geo.difference(interior, geo.rotate(geo.translate(geo.rectangle(1.05, 9.76), [0.475+J2.posX, -5.12+J2.posY]), 90.00000001, J2.pos));

board.addShape("interior", interior);


/* -- ADD_WIRES -- */
board.wire([J1.pad("CLK"),
            pt(IC1.padX("CLK"), J1.padY("CLK")),
            IC1.pad("CLK")], w);

board.wire([J1.pad("DIO"),
            pt(IC1.padX("DIO"), J1.padY("DIO")),
            IC1.pad("DIO")], w);

board.wire([J1.pad("RST"),
            pt(IC1.padX("RST")-.07, J1.padY("RST")),
            pt(IC1.padX("RST")-.07, IC1.padY("RST")),
            IC1.pad("RST")], w);

board.wire([J2.pad("D-"),
            pt(IC1.posX-.015, J2.padY("D-")),
            pt(IC1.posX-.015, IC1.padY("24-")),
            IC1.pad("24-")], w);

board.wire([J2.pad("D+"),
            pt(J2.padX("D+"), J2.padY("D+")-.11),
            pt(IC1.posX+.015, J2.padY("D+")-.11),
            pt(IC1.posX+.015, IC1.padY("25+")),
            IC1.pad("25+")], w);

board.wire([J2.pad("GND"),
            pt(J2.padX("GND"), J2.padY("GND")-.11),
            pt(J2.padX("D+")+.06, J2.padY("GND")-.11),
            pt(J2.padX("D+")+.03, J2.padY("GND")-.14),
            pt(IC1.posX+.045, J2.padY("GND")-.14),
            pt(IC1.posX+.045, IC1.padY("GND")),
            IC1.pad("GND")], w);

board.wire([J2.pad("GND"),
            pt(J2.padX("GND"), J2.padY("GND")-.11),
            pt(IC2.padX("gnd"), J2.padY("GND")-.11),
            IC2.pad("gnd")], w);

board.wire([IC1.pad("VDD"),
            pt(C1.padX("1")-.08, IC1.padY("VDD")),
            pt(C1.padX("1")-.08, C1.padY("1")),
            C1.pad("1")], w);

board.wire([IC2.pad("out"),
            pt(C1.padX("1")-.08, IC2.padY("out"))], w);

board.wire([IC2.pad("gnd"),
            C1.pad("2")], w);

board.wire([C1.pad("2"),
            pt(C1.padX("2")+.07, C1.padY("2")),
            pt(C1.padX("2")+.07, J1.padY("GND")),
            J1.pad("GND")], w);

board.wire([C1.pad("2"),
            pt(C1.padX("2")+.07, C1.padY("2")),
            pt(C1.padX("2")+.07, J3.padY("GND")-.18),
            pt(J3.padX("VCC")+.05, J3.padY("GND")-.18),
            pt(J3.padX("VCC")+.05, J3.padY("GND")+.09),
            pt(J3.padX("VCC")-.05, J3.padY("GND")+.09),
            pt(J3.padX("VCC")-.05, J3.padY("GND")-.18),
            pt(J3.padX("GND"), J3.padY("GND")-.18),
            J3.pad("GND")], w);

board.wire([J2.pad("5V"),
            pt(J2.padX("5V"), J2.padY("5V")-.11),
            pt(J3.padX("GND")-.11, J2.padY("5V")-.11),
            pt(J3.padX("GND")-.11, J3.padY("VCC")-.21),
            pt(J3.padX("VCC"), J3.padY("VCC")-.21),
            J3.pad("VCC")], w);

board.wire([J3.pad("VCC"),
            pt(J3.padX("VCC"), J3.padY("VCC")-.21),
            pt(C1.padX("2")+.1, J3.padY("VCC")-.21),
            pt(C1.padX("2")+.1, IC2.padY("in")),
            IC2.pad("in")], w);

board.wire([IC1.pad("A08"),
            pt(J3.padX("GND")-.04, IC1.padY("A08")),
            pt(J3.padX("GND")-.04, IC1.padY("CLK")),
            pt(IC1.padX("RST")-.1, IC1.padY("CLK")),
            pt(IC1.padX("RST")-.1, J3.padY("GND")+.15),
            pt(J3.padX("RTS"), J3.padY("GND")+.15),
            J3.pad("RTS")], w);

board.wire([IC1.pad("A05"),
            pt(J3.padX("GND")-.07, IC1.padY("A05")),
            pt(J3.padX("GND")-.07, R1.padY("2")),
            R1.pad("2")], w);

board.wire([R1.pad("1"),
            pt(R1.posX, J3.padY("GND")+.12),
            pt(J3.padX("Rx"), J3.padY("GND")+.12),
            J3.pad("Rx")], w);

board.wire([V1.pad("F"),
            IC1.pad("A14")], w);

board.wire([V2.pad("F"),
            J3.pad("CTS")], w);

board.wire([V1.pad("B"),
            V2.pad("B")], w, "B.Cu");

board.wire([V3.pad("F"),
            IC1.pad("A04")], w);

board.wire([V4.pad("F"),
            J3.pad("Tx")], w);

board.wire([V3.pad("B"),
            V4.pad("B")], w, "B.Cu");


// rendering
renderPCB({
  pcb: board,
  layerColors: {
    "interior": "#002d00ff",
    "B.Cu": "#ff4c007f",
    "F.Cu": "#ff8c00cc",
    "drill": "#ff3300e5",
    "padLabels": "#ffff99e5",
    "componentLabels": "#00e5e5e5",
  },
  limits: {
    x: [x-border, x+width+border],
    y: [y-border, y+height+border]
  },
  mm_per_unit: 25.4
})
