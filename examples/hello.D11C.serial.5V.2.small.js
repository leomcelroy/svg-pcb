...
// constants
const width = .78 // board width
const height = 1.41 // board height
const x = 1 // x origin
const y = 1 // y origin
const zt = 0 // top z
const zb = -0.06 // bottom z
const rv = 0.007 // via radius
const rp = 0.014 // pad radius
...
/* -- ADD_COMPONENTS -- */
let IC1 = board.add(SAMD11C, {translate: pt(x+.37, y+height-.63), id: 'IC1\nD11C'});
let J1 = board.add(header_SWD_4_1, {translate: pt(IC1.posX+.015, IC1.padY("CLK")-.14), id: 'J1 SWD'});
let J2 = board.add(USB_A_plug, {translate: pt(x+width/2, y+height-.29), rotate: 90, id: 'J2\nUSB'});
let IC2 = board.add(regulator_SOT23, {translate: pt(IC1.padX("A04")+.17, IC1.padY("A08")-.04), rotate: 90, id: 'IC2\n3.3V'});
let C1 = board.add(C_1206, {translate: pt(IC2.posX, IC2.posY-.2), rotate: 90, id: 'C1\n1uF'});
let J3 = board.add(header_serial_reverse, {translate: pt(IC1.posX+.02, y+.23), rotate: -90, id: 'J3 serial'});
let R1 = board.add(R_1206, {translate: pt(J1.posX-.29, J1.posY), rotate: 90, id: 'R1\n4.99k'});
let V1 = board.add(via(2*rv, 2*rp), {translate: pt(IC1.padX("A14")-.1, IC1.padY("A14")), id: 'V1'});
let V2 = board.add(via(2*rv, 2*rp), {translate: pt(J3.padX("CTS")-.025, J3.padY("CTS")-.11), id: 'V2'});
let V3 = board.add(via(2*rv, 2*rp), {translate: pt(IC1.padX("A04")+.1, IC1.padY("A04")), id: 'V3'});
let V4 = board.add(via(2*rv, 2*rp), {translate: pt(J3.padX("Tx")+.025, J3.padY("Tx")-.11), id: 'V4'});
...
board.wire(path(J1.pad("RST"),
                ...
                [IC1.padX("RST")-.07, IC1.padY("RST")],
                IC1.pad("RST"),), w);

board.wire(path(J2.pad("D-"),
                [IC1.posX-.015, J2.padY("D-")],
                [IC1.posX-.015, IC1.padY("24-")],
                IC1.pad("24-"),), w);

board.wire(path(J2.pad("D+"),
                [J2.padX("D+"), J2.padY("D+")-.11],
                [IC1.posX+.015, J2.padY("D+")-.11],
                [IC1.posX+.015, IC1.padY("25+")],
                IC1.pad("25+"),), w);

board.wire(path(J2.pad("GND"),
                [J2.padX("GND"), J2.padY("GND")-.11],
                [J2.padX("D+")+.06, J2.padY("GND")-.11],
                [J2.padX("D+")+.03, J2.padY("GND")-.14],
                [IC1.posX+.045, J2.padY("GND")-.14],
                [IC1.posX+.045, IC1.padY("GND")],
                IC1.pad("GND"),), w);

board.wire(path(J2.pad("GND"),
                [J2.padX("GND"), J2.padY("GND")-.11],
                [IC2.padX("gnd"), J2.padY("GND")-.11],
                IC2.pad("gnd"),), w);

board.wire(path(IC1.pad("VDD"),
                [C1.padX("1")-.08, IC1.padY("VDD")],
                [C1.padX("1")-.08, C1.padY("1")],
                C1.pad("1"),), w);

board.wire(path(IC2.pad("out"),
                [C1.padX("1")-.08, IC2.padY("out")],), w);

board.wire(path(IC2.pad("gnd"),
                C1.pad("2"),), w);

board.wire(path(C1.pad("2"),
                [C1.padX("2")+.07, C1.padY("2")],
                [C1.padX("2")+.07, J1.padY("GND")],
                J1.pad("GND"),), w);

board.wire(path(C1.pad("2"),
                [C1.padX("2")+.07, C1.padY("2")],
                [C1.padX("2")+.07, J3.padY("GND")-.18],
                [J3.padX("VCC")+.05, J3.padY("GND")-.18],
                [J3.padX("VCC")+.05, J3.padY("GND")+.09],
                [J3.padX("VCC")-.05, J3.padY("GND")+.09],
                [J3.padX("VCC")-.05, J3.padY("GND")-.18],
                [J3.padX("GND"), J3.padY("GND")-.18],
                J3.pad("GND"),), w);

board.wire(path(J2.pad("5V"),
                [J2.padX("5V"), J2.padY("5V")-.11],
                [J3.padX("GND")-.11, J2.padY("5V")-.11],
                [J3.padX("GND")-.11, J3.padY("VCC")-.21],
                [J3.padX("VCC"), J3.padY("VCC")-.21],
                J3.pad("VCC"),), w);

board.wire(path(J3.pad("VCC"),
                [J3.padX("VCC"), J3.padY("VCC")-.21],
                [C1.padX("2")+.1, J3.padY("VCC")-.21],
                [C1.padX("2")+.1, IC2.padY("in")],
                IC2.pad("in"),), w);

board.wire(path(IC1.pad("A08"),
                [J3.padX("GND")-.04, IC1.padY("A08")],
                [J3.padX("GND")-.04, IC1.padY("CLK")],
                [IC1.padX("RST")-.1, IC1.padY("CLK")],
                [IC1.padX("RST")-.1, J3.padY("GND")+.15],
                [J3.padX("RTS"), J3.padY("GND")+.15],
                J3.pad("RTS"),), w);

board.wire(path(IC1.pad("A05"),
                [J3.padX("GND")-.07, IC1.padY("A05")],
                [J3.padX("GND")-.07, R1.padY("2")],
                R1.pad("2"),), w);

board.wire(path(R1.pad("1"),
                [R1.posX, J3.padY("GND")+.12],
                [J3.padX("Rx"), J3.padY("GND")+.12],
                J3.pad("Rx"),), w);
