...
const I2C4x1 = {"VCC":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[-0.065,-0.15],"layers":["F.Cu"],"index":1},"GND":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[0.065,-0.05],"layers":["F.Cu"],"index":2},"SCL":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[-0.065,0.05],"layers":["F.Cu"],"index":3},"SDA":{"shape":"M -0.0395,0.0195L 0.0395,0.0195L 0.0395,-0.0195L -0.0395,-0.0195L -0.0395,0.0195","pos":[0.065,0.15],"layers":["F.Cu"],"index":4}};
...
// constants
const width = 1.19 // board width
const height = .78 // board height
const x = 1 // x origin
const y = 1 // y origin
const zt = 0 // top z
...
/* -- ADD_COMPONENTS -- */
let IC1 = board.add(ATtiny45_SOIC, {translate: pt(x+.53, y+.26), rotate: -90, name: 'IC1\nt45'});
...
let C1 = board.add(C_1206, {translate: pt(R1.posX+.12, R1.posY), rotate: 90, name: 'C1\n1uF'});
let J2 = board.add(header_FTDI, {translate: pt(x+width-.23, C1.posY+.05), name: 'J2 FTDI\n5V'});
let oled = board.add(I2C4x1, {translate: pt(IC1.padX("PB3"), J2.padY("GND")+.05), rotate: 90, name: 'SSD1306'});
let R2 = board.add(R_1206, {translate: pt(J1.posX+.06, oled.padY("SCL")), rotate: 90, name: 'R2\n10k'});
let R3 = board.add(R_1206, {translate: pt(J1.posX-.06, oled.padY("SCL")), rotate: 90, name: 'R3\n10k'});
...
board.wire(path(IC1.pad("VCC"),
                pt(R1.posX, IC1.padY("VCC")),
                R1.pad("1"),), w);

board.wire(path(IC1.pad("RST"),
                pt(IC1.padX("RST"), IC1.padY("RST")-.07),
                pt(R1.posX, IC1.padY("RST")-.07),
                R1.pad("2"),), w);
...
board.wire(path(C1.pad("1"),
                ...
                pt(C1.posX+.06, J2.padY("VCC")),
                J2.pad("VCC"),), w);

board.wire(path(J2.pad("Rx"),
                pt(J2.posX, J2.padY("Rx")-.05),
                pt(C1.posX, J2.padY("Rx")-.05),
                pt(C1.posX, IC1.padY("PB2")-.08),
                pt(IC1.padX("PB2"), IC1.padY("PB2")-.08),
                IC1.pad("PB2"),), w);

board.wire(path(oled.pad("VCC"),
                pt((R1.posX+C1.posX)/2, oled.padY("VCC")),
                pt((R1.posX+C1.posX)/2, C1.padY("1")),), w);

board.wire(path(oled.pad("GND"),
                pt(J2.posX, oled.padY("GND")),
                J2.pad("GND"),), w);

board.wire(path(oled.pad("SCL"),
                IC1.pad("PB4"),), w);

board.wire(path(oled.pad("SDA"),
                pt(oled.posX, oled.padY("SDA")),
                IC1.pad("PB3"),), w);

board.wire(path(R2.pad("2"),
                pt(oled.padX("SCL"), R2.padY("2")),
                oled.pad("SCL"),), w);
...
board.wire(path(R3.pad("1"),
                pt(J1.padX("MOSI")-.08, R3.padY("1")),
                pt(J1.padX("MOSI")-.08, J1.padY("MOSI")),), w);

board.wire(path(R3.pad("2"),
                pt(R3.posX, oled.padY("SDA")),
                oled.pad("SDA"),), w);

board.wire(path(oled.pad("GND"),
                pt(oled.padX("GND"), oled.padY("GND")+.07),
                pt(J1.padX("GND")-.08, oled.padY("GND")+.07),
                pt(J1.padX("GND")-.08, R3.posY),
                pt(IC1.padX("GND"), R3.posY),
                IC1.pad("GND"),), w);
