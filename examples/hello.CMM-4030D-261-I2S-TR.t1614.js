const ATtiny1614 = footprint({"VCC":{"shape":"M 0.03,0.015L -0.03,0.015L -0.0305,0.015L -0.031,0.015L -0.0321,0.0149L -0.0331,0.0147L -0.0336,0.0146L -0.0347,0.0142L -0.0356,0.0139L -0.0361,0.0137L -0.0366,0.0135L -0.0371,0.0132L -0.0375,0.013L -0.038,0.0127L -0.0384,0.0125L -0.0392,0.0118L -0.0396,0.0115L -0.0401,0.0112L -0.0412,0.0101L -0.0415,0.0096L -0.0418,0.0092L -0.0425,0.0084L -0.0427,0.008L -0.043,0.0075L -0.0432,0.0071L -0.0435,0.0066L -0.0437,0.0061L -0.0439,0.0056L -0.0442,0.0047L -0.0446,0.0036L -0.0447,0.0031L -0.0449,0.0021L -0.045,0.001L -0.045,-0.001L -0.0449,-0.0021L -0.0447,-0.0031L -0.0446,-0.0036L -0.0442,-0.0047L -0.0439,-0.0056L -0.0437,-0.0061L -0.0435,-0.0066L -0.0432,-0.0071L -0.043,-0.0075L -0.0427,-0.008L -0.0425,-0.0084L -0.0418,-0.0092L -0.0415,-0.0096L -0.0412,-0.0101L -0.0401,-0.0112L -0.0396,-0.0115L -0.0392,-0.0118L -0.0384,-0.0125L -0.038,-0.0127L -0.0375,-0.013L -0.0371,-0.0132L -0.0366,-0.0135L -0.0361,-0.0137L -0.0356,-0.0139L -0.0347,-0.0142L -0.0336,-0.0146L -0.0331,-0.0147L -0.0321,-0.0149L -0.031,-0.015L 0.03,-0.015L 0.03,0.015","pos":[-0.11,0.15],"layers":["F.Cu","F.Mask"],"index":1},"PA4":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[-0.11,0.1],"layers":["F.Cu","F.Mask"],"index":2},"PA5":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[-0.11,0.05],"layers":["F.Cu","F.Mask"],"index":3},"PA6":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[-0.11,0],"layers":["F.Cu","F.Mask"],"index":4},"PA7":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[-0.11,-0.05],"layers":["F.Cu","F.Mask"],"index":5},"RB3":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[-0.11,-0.1],"layers":["F.Cu","F.Mask"],"index":6},"TB2":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[-0.11,-0.15],"layers":["F.Cu","F.Mask"],"index":7},"PB1":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,-0.15],"layers":["F.Cu","F.Mask"],"index":8},"PB0":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,-0.1],"layers":["F.Cu","F.Mask"],"index":9},"UPDI":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,-0.05],"layers":["F.Cu","F.Mask"],"index":10},"PA1":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,0],"layers":["F.Cu","F.Mask"],"index":11},"PA2":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,0.05],"layers":["F.Cu","F.Mask"],"index":12},"PA3":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,0.1],"layers":["F.Cu","F.Mask"],"index":13},"GND":{"shape":"M -0.03,0.015L 0.03,0.015L 0.03,-0.015L -0.03,-0.015L -0.03,0.015","pos":[0.11,0.15],"layers":["F.Cu","F.Mask"],"index":14}});
const header_FTDI = footprint({"GND":{"shape":"M 0.05,0.025L -0.05,0.025L -0.0509,0.025L -0.0517,0.025L -0.0535,0.0248L -0.0552,0.0244L -0.056,0.0243L -0.0578,0.0237L -0.0593,0.0232L -0.0602,0.0229L -0.061,0.0225L -0.0618,0.0221L -0.0625,0.0216L -0.0633,0.0212L -0.064,0.0208L -0.0654,0.0197L -0.0661,0.0191L -0.0668,0.0186L -0.0686,0.0168L -0.0691,0.0161L -0.0697,0.0154L -0.0708,0.014L -0.0712,0.0133L -0.0716,0.0125L -0.0721,0.0118L -0.0725,0.011L -0.0729,0.0102L -0.0732,0.0093L -0.0737,0.0078L -0.0743,0.006L -0.0744,0.0052L -0.0748,0.0035L -0.075,0.0017L -0.075,0.0009L -0.075,0L -0.075,-0.0009L -0.075,-0.0017L -0.0748,-0.0035L -0.0744,-0.0052L -0.0743,-0.006L -0.0737,-0.0078L -0.0732,-0.0093L -0.0729,-0.0102L -0.0725,-0.011L -0.0721,-0.0118L -0.0716,-0.0125L -0.0712,-0.0133L -0.0708,-0.014L -0.0697,-0.0154L -0.0691,-0.0161L -0.0686,-0.0168L -0.0668,-0.0186L -0.0661,-0.0191L -0.0654,-0.0197L -0.064,-0.0208L -0.0633,-0.0212L -0.0625,-0.0216L -0.0618,-0.0221L -0.061,-0.0225L -0.0602,-0.0229L -0.0593,-0.0232L -0.0578,-0.0237L -0.056,-0.0243L -0.0552,-0.0244L -0.0535,-0.0248L -0.0517,-0.025L 0.05,-0.025L 0.05,0.025","pos":[0,0.25],"layers":["F.Cu","F.Mask"],"index":1},"CTS":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,0.15],"layers":["F.Cu","F.Mask"],"index":2},"VCC":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,0.05],"layers":["F.Cu","F.Mask"],"index":3},"Tx":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,-0.05],"layers":["F.Cu","F.Mask"],"index":4},"Rx":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,-0.15],"layers":["F.Cu","F.Mask"],"index":5},"RTS":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,-0.25],"layers":["F.Cu","F.Mask"],"index":6}});
const regulator_SOT23 = footprint({"out":{"shape":"M -0.02,0.012L 0.02,0.012L 0.02,-0.012L -0.02,-0.012L -0.02,0.012","pos":[-0.045,0.0375],"layers":["F.Cu","F.Mask"],"index":1},"in":{"shape":"M -0.02,0.012L 0.02,0.012L 0.02,-0.012L -0.02,-0.012L -0.02,0.012","pos":[-0.045,-0.0375],"layers":["F.Cu","F.Mask"],"index":2},"gnd":{"shape":"M -0.02,0.012L 0.02,0.012L 0.02,-0.012L -0.02,-0.012L -0.02,0.012","pos":[0.045,0],"layers":["F.Cu","F.Mask"],"index":3}});
const C_1206 = footprint({"1":{"shape":"M -0.032,0.034L 0.032,0.034L 0.032,-0.034L -0.032,-0.034L -0.032,0.034","pos":[-0.06,0],"layers":["F.Cu","F.Mask"],"index":1},"2":{"shape":"M -0.032,0.034L 0.032,0.034L 0.032,-0.034L -0.032,-0.034L -0.032,0.034","pos":[0.06,0],"layers":["F.Cu","F.Mask"],"index":2}});
const header_UPDI = footprint({"UPDI":{"shape":"M -0.05,-0.025L 0.0517,-0.025L 0.0535,-0.0248L 0.0552,-0.0244L 0.056,-0.0243L 0.0578,-0.0237L 0.0593,-0.0232L 0.0602,-0.0229L 0.061,-0.0225L 0.0618,-0.0221L 0.0625,-0.0216L 0.0633,-0.0212L 0.064,-0.0208L 0.0654,-0.0197L 0.0661,-0.0191L 0.0668,-0.0186L 0.0686,-0.0168L 0.0691,-0.0161L 0.0697,-0.0154L 0.0708,-0.014L 0.0712,-0.0133L 0.0716,-0.0125L 0.0721,-0.0118L 0.0725,-0.011L 0.0729,-0.0102L 0.0732,-0.0093L 0.0737,-0.0078L 0.0743,-0.006L 0.0744,-0.0052L 0.0748,-0.0035L 0.075,-0.0017L 0.075,-0.0009L 0.075,0L 0.075,0.0009L 0.075,0.0017L 0.0748,0.0035L 0.0744,0.0052L 0.0743,0.006L 0.0737,0.0078L 0.0732,0.0093L 0.0729,0.0102L 0.0725,0.011L 0.0721,0.0118L 0.0716,0.0125L 0.0712,0.0133L 0.0708,0.014L 0.0697,0.0154L 0.0691,0.0161L 0.0686,0.0168L 0.0668,0.0186L 0.0661,0.0191L 0.0654,0.0197L 0.064,0.0208L 0.0633,0.0212L 0.0625,0.0216L 0.0618,0.0221L 0.061,0.0225L 0.0602,0.0229L 0.0593,0.0232L 0.0578,0.0237L 0.056,0.0243L 0.0552,0.0244L 0.0535,0.0248L 0.0517,0.025L 0.0509,0.025L 0.05,0.025L -0.05,0.025L -0.05,-0.025","pos":[0,-0.05],"layers":["F.Cu","F.Mask"],"index":1},"GND":{"shape":"M -0.05,0.025L 0.05,0.025L 0.05,-0.025L -0.05,-0.025L -0.05,0.025","pos":[0,0.05],"layers":["F.Cu","F.Mask"],"index":2}});
const CMM4030D261I2STR = footprint({"GND":{"shape":"M -0.018,0.011L 0.018,0.011L 0.018,-0.011L -0.018,-0.011L -0.018,0.011","pos":[0.037,-0.059],"layers":["F.Cu","F.Mask"],"index":1},"N/C":{"shape":"M -0.018,0.011L 0.018,0.011L 0.018,-0.011L -0.018,-0.011L -0.018,0.011","pos":[0.037,-0.019],"layers":["F.Cu","F.Mask"],"index":2},"WS":{"shape":"M -0.018,0.011L 0.018,0.011L 0.018,-0.011L -0.018,-0.011L -0.018,0.011","pos":[0.037,0.019],"layers":["F.Cu","F.Mask"],"index":3},"EN":{"shape":"M -0.018,0.011L 0.018,0.011L 0.018,-0.011L -0.018,-0.011L -0.018,0.011","pos":[0.037,0.059],"layers":["F.Cu","F.Mask"],"index":4},"L/R":{"shape":"M -0.018,0.011L 0.018,0.011L 0.018,-0.011L -0.018,-0.011L -0.018,0.011","pos":[-0.037,0.059],"layers":["F.Cu","F.Mask"],"index":5},"SCK":{"shape":"M -0.018,0.011L 0.018,0.011L 0.018,-0.011L -0.018,-0.011L -0.018,0.011","pos":[-0.037,0.019],"layers":["F.Cu","F.Mask"],"index":6},"SD":{"shape":"M -0.018,0.011L 0.018,0.011L 0.018,-0.011L -0.018,-0.011L -0.018,0.011","pos":[-0.037,-0.019],"layers":["F.Cu","F.Mask"],"index":7},"VDD":{"shape":"M -0.018,0.011L 0.018,0.011L 0.018,-0.011L -0.018,-0.011L -0.018,0.011","pos":[-0.037,-0.059],"layers":["F.Cu","F.Mask"],"index":8}});
const R_1206 = footprint({"1":{"shape":"M -0.032,0.034L 0.032,0.034L 0.032,-0.034L -0.032,-0.034L -0.032,0.034","pos":[-0.06,0],"layers":["F.Cu","F.Mask"],"index":1},"2":{"shape":"M -0.032,0.034L 0.032,0.034L 0.032,-0.034L -0.032,-0.034L -0.032,0.034","pos":[0.06,0],"layers":["F.Cu","F.Mask"],"index":2}});


// @version: v0.1.0
// commands:
//  - SHIFT+ENTER: render
//  - drap & drop file: kicad import


// constants
const width = .98 // board width
const height = .9 // board height
const x = 1 // x origin
const y = 1 // y origin
const zt = 0 // top z
const zb = -0.06 // bottom z
const w = .015 // wire width
const mask = .004 // solder mask size
const border = 0.05 // rendering border


/* -- DECLARE_PCB -- */
let board = new PCB();



/* -- ADD_COMPONENTS -- */
let IC1 = board.add(ATtiny1614, {translate: pt(x+.57, y+.55), label: 'IC1\nt1614'});
let J1 = board.add(header_FTDI, {translate: pt(x+.23, IC1.posY), rotate: 180, label: 'J1\nFTDI'});
let IC2 = board.add(regulator_SOT23, {translate: pt(J1.posX, J1.padY("GND")-.13), rotate: -90, label: 'IC2\n3.3V'});
let C1 = board.add(C_1206, {translate: pt(IC2.posX+.15, IC2.posY), rotate: 90, label: 'C1\n1uF'});
let J2 = board.add(header_UPDI, {translate: pt(IC1.posX, y+.23), rotate: 90, label: 'J2\nUPDI'});
let IC3 = board.add(CMM4030D261I2STR, {translate: pt(IC1.posX+.26, IC1.posY-.014), label: 'I2S'});
let C2 = board.add(C_1206, {translate: pt(IC3.posX+.04, IC3.posY-.15), label: 'C2 .1u'});
let C3 = board.add(C_1206, {translate: pt(C2.posX, C2.posY-.1), label: 'C3 10u'});
let R1 = board.add(R_1206, {translate: pt(J2.posX+.145, J2.posY+.02), rotate: 90, label: 'R1\n100k'});

let interior = geo.path(path([1.000, 1.900],
                             [1.980, 1.900],
                             [1.980, 1.000],
                             [1.000, 1.000]));

board.addShape("interior", interior);


/* -- ADD_WIRES -- */
board.wire(path(IC1.pad("RB3"),
                [IC1.padX("RB3")-.08, IC1.padY("RB3")],
                [IC1.padX("RB3")-.08, J1.padY("RTS")+.06],
                [J1.posX-.1, J1.padY("RTS")+.06],
                [J1.posX-.1, J1.padY("Tx")],
                J1.pad("Tx"),), w);

board.wire(path(IC1.pad("TB2"),
                [IC1.padX("TB2")-.11, IC1.padY("TB2")],
                [IC1.padX("TB2")-.11, J1.padY("Rx")],
                J1.pad("Rx"),), w);

board.wire(path(IC2.pad("in"),
                [J1.posX-.1, IC2.padY("in")],
                [J1.posX-.1, J1.padY("VCC")],
                J1.pad("VCC"),), w);

board.wire(path(IC2.pad("gnd"),
                J1.pad("GND"),), w);

board.wire(path(C1.pad("1"),
                [IC2.posX, C1.padY("1")],
                IC2.pad("gnd"),), w);

board.wire(path(C1.pad("2"),
                [IC2.padX("out"), C1.padY("2")],
                IC2.pad("out"),), w);

board.wire(path(C1.pad("2"),
                [C1.padX("2"), IC1.padY("TB2")-.05],
                [IC1.posX-.045, IC1.padY("TB2")-.05],
                [IC1.posX-.045, IC1.padY("VCC")],
                IC1.pad("VCC"),), w);

board.wire(path(IC1.pad("UPDI"),
                [IC1.posX+.015, IC1.padY("UPDI")],
                [IC1.posX+.015, J2.posY+.09],
                [J2.padX("UPDI"), J2.posY+.03],
                J2.pad("UPDI"),), w);

board.wire(path(IC1.pad("GND"),
                [IC1.posX-.015, IC1.padY("GND")],
                [IC1.posX-.015, J2.posY+.09],
                [J2.padX("GND"), J2.posY+.03],
                J2.pad("GND"),), w);

board.wire(path(C1.pad("1"),
                [J2.padX("GND"), C1.padY("1")],
                J2.pad("GND"),), w);

board.wire(path(IC3.pad("GND"),
                [IC3.padX("GND")+.08, IC3.padY("GND")],
                [IC3.padX("GND")+.08, IC1.padY("GND")],
                IC1.pad("GND"),), w);

board.wire(path(IC3.pad("WS"),
                [IC3.padX("WS")+.05, IC3.padY("WS")],
                [IC3.padX("WS")+.05, IC1.padY("PA3")],
                IC1.pad("PA3"),), w);

board.wire(path(IC3.pad("EN"),
                IC3.pad("L/R"),), w);

board.wire(path(IC3.pad("L/R"),
                [IC1.padX("PA2"), IC3.padY("L/R")],
                IC1.pad("PA2"),), w);

board.wire(path(IC3.pad("SCK"),
                [IC1.padX("PA1"), IC3.padY("SCK")],
                IC1.pad("PA1"),), w);

board.wire(path(IC3.pad("SD"),
                [IC3.padX("SD")-.05, IC3.padY("SD")],
                [IC3.padX("SD")-.05, IC1.padY("PB0")],
                IC1.pad("PB0"),), w);

board.wire(path(C2.pad("1"),
                [IC3.padX("VDD"), C2.padY("1")],
                IC3.pad("VDD"),), w);

board.wire(path(C2.pad("2"),
                [C2.padX("2"), IC3.padY("GND")],
                IC3.pad("GND"),), w);

board.wire(path(C3.pad("1"),
                C2.pad("1"),), w);

board.wire(path(C1.pad("2"),
                [C1.posX, IC1.padY("TB2")-.05],
                [IC1.padX("TB2")-.14, IC1.padY("TB2")-.05],
                [IC1.padX("TB2")-.14, J1.padY("VCC")+.05],
                [J1.posX-.13, J1.padY("VCC")+.05],
                [J1.posX-.13, C1.padY("1")-.07],
                [C3.padX("1"), C1.padY("1")-.07],
                C3.pad("1"),), w);

board.wire(path(C3.pad("2"),
                C2.pad("2"),), w);

board.wire(path(C1.pad("1"),
                [R1.padX("1"), C1.padY("1")],
                R1.pad("1"),), w);

board.wire(path(IC3.pad("SD"),
                [IC3.padX("SD")-.05, IC3.padY("SD")],
                [IC3.padX("SD")-.05, R1.padY("2")],
                R1.pad("2"),), w);


// rendering
renderPCB({
  pcb: board,
  layerColors: {
    "interior": "#002d00ff",
    "B.Cu": "#ff4c007f",
    "F.Cu": "#be7a27cc",
    "B.Mask": "#ff814bff",
    "F.Mask": "#ffa50aff",
    "padLabels": "#ffff99e5",
    "componentLabels": "#00e5e5e5",
  },
  limits: {
    x: [x-border, x+width+border],
    y: [y-border, y+height+border]
  },
  mm_per_unit: 25.4
})
