...
`);function X(e){return q`
	...
	`}var me=e=>e.length===4?`rgba(${e.map((r,t)=>t<3?Math.round(r*255):r).join(",")})`:"rgba(255, 255, 255, 1)",U=(e,r)=>P`
	<path
		d="${e}"
		fill-rule="nonzero"
		fill="${typeof r=="string"?r:me(r)}"/>
`,pe=e=>e.reduce((r,t,o)=>`${r} ${o===0?"M":"L"} ${t.join(",")}`,""),he=(e,r)=>{let t=[[e.lt.x,(e.lt.y+e.lb.y)/2],[e.rt.x,(e.rt.y+e.rb.y)/2]],o=[[(e.lt.x+e.rt.x)/2,e.lt.y],[(e.lb.x+e.rb.x)/2,e.lb.y]],s=[[e.lt.x,0],[e.rt.x,0]],i=[[0,e.lt.y],[0,e.lb.y]],n=[e.lt.x,e.rt.x],m=Math.abs(n[1]-n[0]),l=[e.lb.y,e.lt.y],c=Math.abs(l[1]-l[0]);function h(b,u){return Math.log(u)/Math.log(b)}let a=Math.round(h(5,Math.max(m,c))),d=r,p=(b,u)=>{let j=[],k=u*Math.ceil(b[0]/u)-u;for(;k<b[1]+u;)k=k+u,j.push(k);return j},_=b=>[[e.lt.x,b],[e.rt.x,b]],w=b=>[[b,e.lt.y],[b,e.lb.y]],f=b=>u=>P`<path stroke="black" vector-effect="non-scaling-stroke" stroke-width="${b}" d="${pe(u)}"/>`,v=p(l,d).map(_).map(f(.2)),x=p(n,d).map(w).map(f(.2));return P`
	...
	`},ue=e=>{var o;let r=e.shapes.map(s=>Array.isArray(s.d)?s.d.map(i=>U(i,s.color)):U(s.d,s.color)).flat(),t=(o=e.panZoomParams)==null?void 0:o.corners();return P`
	...
	`},C=.02,fe=e=>e.components.map((r,t)=>P`
	<g class="no-download translate-handle">
		<path
	        d="${new ce().arc(361,C).translate([r.posX,r.posY-C]).offset(.003).getPathData()}"
	  	/>
	  	<circle
...
`);import{render as Oe}from"https://cdn.skypack.dev/lit-html";import ge from"https://cdn.skypack.dev/esprima";import"https://cdn.skypack.dev/astring";import{walk as D}from"https://cdn.skypack.dev/esprima-walk";function ye(e){let r=n=>n.includes("return kicadToObj("),o=e.codemirror.view.state.doc.toString().split(`
`),s=0,i=0;for(;;){let n=o[s];if(!n||(i+=n.length,s>o.length)||o[s]==="`)})()"&&!r(o[s+1]))break;s++}e.codemirror.foldRange(0,i+s)}function V(e,r){let t=document.querySelector("svg"),o=!1,s,i=0,n=0,m;r("mousedown",".translate-handle-trigger",l=>{o=!0,e.transforming=!0,i=0,n=0,s=t.panZoomParams.svgPoint({x:l.offsetX,y:l.offsetY}),m=l.target.dataset.index;let h=e.codemirror.view.state.doc.toString(),a=`()=>{${h}}`,d=ge.parseScript(a,{range:!0,comment:!0});console.log(d);let p=d.body[0].expression.body.body;console.log(p);let _=[];D(d,u=>{try{u.callee.type==="MemberExpression"&&u.callee.property.name==="add"&&_.push(u.arguments[1])}catch(j){}});let w=_.sort((u,j)=>u.range[0]-j.range[0]),f=u=>u.includes(".")?u.split(".")[1].length:u.length,v=(u,j)=>Math.round(u/j)*j,x=(u,j)=>Math.round(u*10**j)/10**j,b=u=>/[0-9]/i.test(u)||u===".";e.transformUpdate=(u,j)=>{let k=[];w[m].properties.forEach(M=>{if(M.key.name!=="translate")return;let[ie,ae]=M.value.elements,L=!1;D(ie,g=>{if(!L&&g.type==="Literal"&&typeof g.value=="number"){g.ogValue||(g.ogValue=g.value),g.ogRaw||(g.ogRaw=g.raw);let $=g.ogValue+u;k.push({from:g.range[0]-5,to:g.range[1]-5,insert:`${e.gridSize===0?x($,f(g.ogRaw)):x(v($,e.gridSize),8)}`}),L=!0}});let I=!1;D(ae,g=>{if(!I&&g.type==="Literal"&&typeof g.value=="number"){g.ogValue||(g.ogValue=g.value),g.ogRaw||(g.ogRaw=g.raw);let $=g.ogValue+j;k.push({from:g.range[0]-5,to:g.range[1]-5,insert:`${e.gridSize===0?x($,f(g.ogRaw)):x(v($,e.gridSize),8)}`}),I=!0}});let le=e.codemirror.view.state.doc.toString();e.codemirror.view.dispatch({changes:{from:0,to:le.length,insert:h}}),e.codemirror.view.dispatch({changes:k})}),ye(e)}}),r("mousemove","",l=>{if(!o)return;let h=t.panZoomParams.svgPoint({x:l.offsetX,y:l.offsetY}),a=h.x-s.x,d=h.y-s.y;dispatch("TRANSLATE",{x:a,y:d,index:m}),i=a,n=d}),r("mouseup","",l=>{o=!1,e.transforming=!1}),r("mouseleave","",l=>{o=!1,e.transforming=!1})}function F(e,r){let t=!1;r("mousedown","#vertical-bar",o=>{t=!0}),r("mousemove","",o=>{if(!t)return;let s=o.clientX/window.innerWidth*100;if(s===0)return;let i=1,n=99;s<i&&(s=i),s>n&&(s=n),document.documentElement.style.setProperty("--vertical-bar",`${s}%`),pauseEvent(o)}),r("mouseup","",o=>{t=!1})}function z(e,r){let t=!1,o=1,s=0,i=0,n={x:0,y:0};function m(a){a.style.transformOrigin=`${0}px ${0}px`,a.style.transform="translate("+s+"px, "+i+"px) scale("+o+")",e.gridSize>0&&dispatch("RENDER")}function l({x:a,y:d}){let p=(a-s)/o,_=(d-i)/o;return{x:p,y:_}}r("mousedown","",a=>{a.shiftKey||(t=!0,n={x:a.offsetX-s,y:a.offsetY-i},a.detail===2&&console.log(a.offsetX,a.offsetY,l({x:a.offsetX,y:a.offsetY})))}),r("mousemove","",a=>{if(!t||e.transforming)return;s=a.offsetX-n.x,i=a.offsetY-n.y;let d=document.querySelectorAll(".transform-group");for(let p of d)m(p)}),r("mouseup","",a=>{t=!1}),r("wheel","",a=>{let d=(a.offsetX-s)/o,p=(a.offsetY-i)/o;Math.sign(a.deltaY)<0?o*=1.03:o/=1.03,s=a.offsetX-d*o,i=a.offsetY-p*o;let _=document.querySelectorAll(".transform-group");for(let w of _)m(w);a.preventDefault()});function c(a){console.log("set scale x y");let p=document.querySelector("svg").getBoundingClientRect(),_=a.x[1]-a.x[0],w=a.y[1]-a.y[0],f=p.width/_,v=p.height/w,x=Math.min(f,v)*.9;o=x;let b={x:(a.x[0]+a.x[1])/2*x-p.width/2,y:(a.y[0]+a.y[1])/2*x-p.height/2};s=-b.x,i=-b.y;let u=document.querySelectorAll(".transform-group");for(let j of u)m(j)}function h(){let a=document.querySelector("svg");if(a===null)return null;let{left:d,right:p,bottom:_,top:w,width:f,height:v}=a.getBoundingClientRect(),x=l({x:f,y:v}),b=l({x:0,y:v}),u=l({x:f,y:0}),j=l({x:0,y:0});return{rt:x,lt:b,rb:u,lb:j}}return{scale:()=>o,x:()=>s,y:()=>i,corners:h,svgPoint:l,setScaleXY:c}}var H=0;function _e(e){var r=new FileReader;r.readAsText(e),r.onloadend=t=>{let o=r.result;dispatch("UPLOAD_COMP",{text:o,name:`component${H}`}),H++}}function be(e){var r=new FileReader;r.readAsText(e),r.onloadend=t=>{let o=r.result;dispatch("UPLOAD_JS",{text:o})}}function ve(e,r=[]){let t=e[0],o=t.name.split("."),s=o[0],i=o[o.length-1];if(i==="kicad_mod")_e(t);else if(i==="js")be(t);else throw Error("Unknown extension:",i)}function Y(e,r){r("drop","",function(t){let s=t.dataTransfer.files;ve(s),pauseEvent(t)}),r("dragover","",function(t){pauseEvent(t)})}function Z(e,r){let t=null,o=null,s=document.querySelector("svg");r("mousedown","",n=>{!n.shiftKey||(t=s.panZoomParams.svgPoint({x:n.offsetX,y:n.offsetY}))}),r("mousemove","",n=>{document.body.classList.add("no-select"),!!n.shiftKey&&t!==null&&(o=s.panZoomParams.svgPoint({x:n.offsetX,y:n.offsetY}),e.selectBox.start=t,e.selectBox.end=o,dispatch("RENDER"))});function i(n,m){let{start:l,end:c}=m;return n.x>l.x&&n.x<c.x&&n.y>l.y&&n.y<c.y||n.x>l.x&&n.x<c.x&&n.y<l.y&&n.y>c.y||n.x<l.x&&n.x>c.x&&n.y>l.y&&n.y<c.y||n.x<l.x&&n.x>c.x&&n.y<l.y&&n.y>c.y}r("mouseup","",n=>{!n.shiftKey||(document.body.classList.remove("no-select"),t=null,o=null,e.selectBox.start=t,e.selectBox.end=o,dispatch("RENDER"))})}var W=(e,r=!1)=>/[0-9]/i.test(e)||e==="."||r&&e==="-";function G(e,r){let t=!1,o,s,i,n,m,l;r("mousedown",".ͼb",c=>{let a=document.querySelector("code-mirror").view.state,d=a.doc,p=a.selection.main.head,_=d.lineAt(p),{from:w,to:f,text:v}=d.lineAt(p),x=p,b=p;for(;x>w&&W(v[x-w-1],!0);)x--;for(;b<f&&W(v[b-w]);)b++;l=v.slice(x-w,b-w),o=Number(l),t=!0,s=x,i=b,m=l.includes("."),n=l.includes(".")?l.split(".")[1].length:1}),r("mousemove","",c=>{if(!t)return;let h=document.querySelector("code-mirror"),a=0>c.movementX?1:-1,d=`${o}`;if(m){let w=Math.round(o*10**n)+c.movementX;w=Math.round(w)/10**n,o=w}else o+=c.movementX;let p=`${o}`;h.view.dispatch({changes:{from:s,to:s+l.length,insert:p}}),l=p,dispatch("RUN"),pauseEvent(c)}),r("mouseup","",c=>{t=!1})}function we(e){return e.stopPropagation&&e.stopPropagation(),e.preventDefault&&e.preventDefault(),e.cancelBubble=!0,e.returnValue=!1,!1}window.pauseEvent=we;var K=e=>e.composedPath()[0],xe=(e,r)=>K(e).matches(r),J=e=>(r,t,o)=>{e.addEventListener(r,s=>{s.trigger=K(s),(t===""||xe(s,t))&&o(s)})};function Q(e){let r=document.querySelector("svg"),t=J(r);r.panZoomParams=z(e,t),e.panZoomParams=r.panZoomParams,Z(e,t),V(e,t);let o=document.querySelector("body"),s=J(o);Y(e,s),G(e,s),F(e,s),s("keydown","",i=>{let n=event.code;n==="Enter"&&event.shiftKey?(event.preventDefault(),dispatch("RUN",{save:!0})):n==="KeyT"&&event.shiftKey})}import{Turtle as y}from"https://leomcelroy.com/gram-js/exports.js";function je(e){return new y().arc(360,e).originate()}function Se(e,r){return new y().forward(e).right(90).forward(r).right(90).repeat(1).originate()}y.prototype.circle=je;y.prototype.rectangle=Se;var Te=([e,r],[t,o])=>Math.sqrt((t-e)**2+(o-r)**2),ke=(e,r)=>Te(e,r)<1e-6,ee=(e,r)=>{let t=e[0],o=new y;o.booleanScale=2e3,o.goto(t,!1);for(let s of e.slice(1))ke(s,t)||(o.goto(s),t=s);return o.offset(r/2)},Pe=([e,r],[t,o])=>[e+t,r+o];var $e=([e,r],t)=>[e*Math.cos(t)-r*Math.sin(t),r*Math.cos(t)+e*Math.sin(t)],re=class{constructor({pads:r,layers:t}){this.pads=r,this.layers=t}pad(r){return this.pads[r]}padX(r){return this.pads[r][0]}padY(r){return this.pads[r][1]}get pos(){return this.pads.center}get posX(){return this.pads.center[0]}get posY(){return this.pads.center[1]}};function te(e,r={}){let t=r.translate||[0,0],o=r.rotate||0,s=r.padLabelSize||2e-4,i=r.componentLabelSize||3e-4,[n,m]=t,l=o*Math.PI/180,c={},h=[],a={};for(let d in e){let{pos:p,shape:_,layers:w}=e[d];_=typeof _=="string"?new y().bezier(_):_.copy();let f=Pe($e(p,l),t);if(c[d]=f,_.translate(f).rotate(o,f),!d.includes("_drill")){let v=new y().text(d).scale(s).originate().translate(f);h.push(v)}w.forEach(v=>{v in a?a[v]=a[v].group(_):a[v]=_})}return c.center=t,a.padLabels=h.reduce((d,p)=>d.group(p),new y),new re({pads:c,layers:a})}var R=class{constructor(){this.layers={},this.components=[]}add(r,t={}){let o=t.name||"",s={translate:t.translate||[0,0],rotate:t.rotate||0,padLabelSize:t.padLabelSize||3e-4,componentLabelSize:t.componentLabelSize||4e-4},i=te(r,s);for(let n in i.layers)this.addShape(n,i.layers[n]);if(o!==""&&!o.includes("_drill")){let n=new y().text(o).scale(s.componentLabelSize).originate().translate(s.translate);this.addShape("componentLabels",n)}return this.components.push(i),i}via(r,t,o,s){let i={"1":{pos:[0,0],shape:new y().circle(o).getPathData(),layers:["F.Cu"],index:1},"2":{pos:[0,0],shape:new y().circle(o).getPathData(),layers:["B.Cu"],index:2},drill:{pos:[0,0],shape:new y().circle(t).getPathData(),layers:["drill"]}};return this.add(i,{translate:r,name:s})}addShape(r,t){return t instanceof y?(r in this.layers?this.layers[r]=this.layers[r].group(t):this.layers[r]=t,this.layers[r]):console.error("Shape isn't Turtle.")}subtractShape(r,t){return t instanceof y?(r in this.layers?this.layers[r]=this.layers[r].difference(t):this.layers[r]=new y,this.layers[r]):console.error("Shape isn't Turtle.")}getLayer(r,t=!1){let o=r.includes("Labels");return t=!r.includes("Labels"),this.layers[r]?t?this.layers[r].flatten().getPathData(o):this.layers[r].getPathData(o):""}wire(r,t,o="F.Cu"){this.addShape(o,ee(r,t))}};function S(e){this._line=this._col=this._pos=0,this._stream=e}S.not_whitespace_or_end=/^(\S|$)/;S.space_quote_paren_escaped_or_end=/^(\s|\\|"|'|`|,|\(|\)|$)/;S.string_or_escaped_or_end=/^(\\|"|$)/;S.string_delimiters=/["]/;S.quotes=/['`,]/;S.quotes_map={"'":"quote","`":"quasiquote",",":"unquote"};S.prototype={peek:Ee,consume:Ce,until:De,error:Re,string:Ae,atom:Me,quoted:Le,expr:Ie,list:Ne};function oe(e){var r=new S(e),t=r.expr();return t instanceof Error?t:r.peek()!=""?r.error("Superfluous characters after expression: `"+r.peek()+"`"):t}function Re(e){var r=new Error("Syntax error: "+e);return r.line=this._line+1,r.col=this._col+1,r}function Ee(){return this._stream.length==this._pos?"":this._stream[this._pos]}function Ce(){if(this._stream.length==this._pos)return"";var e=this._stream[this._pos];return this._pos+=1,e=="\r"?(this.peek()==`
`&&(this._pos+=1,e+=`
...
`,t.codemirror.view.dispatch({changes:{from:0,insert:e}}),T("RENDER")},TRANSLATE({x:e,y:r,index:t},o){o.transformUpdate(e,r),T("RUN")},RENDER(){Oe(X(A),document.getElementById("root"))}};function T(e,r={}){let t=Xe[e];t?t(r,A):console.log("Action not recongnized:",e)}window.dispatch=T;window.addEventListener("load",()=>{T("INIT")});export{R as PCB,y as Turtle,E as kicadToObj};
