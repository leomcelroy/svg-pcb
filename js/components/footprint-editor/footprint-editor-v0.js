        ...
        this.panZoomFns = addPanZoom(
            ...
            ({ scale, pointX, pointY }) => {
                ...
                for (const el of fixedScales) {
                    ...
                    el.style.transform = `translate(${pointX}px, ${pointY}px)`;
                }

                // this.render();
                const transformHandle = this.querySelector("#transform-handle");
                if (transformHandle) {
                    const centerX = Number(transformHandle.dataset.centerX);
                    const centerY = Number(transformHandle.dataset.centerY);
                    transformHandle.style=`transform: 
                        translate(
                            ${centerX*scale + pointX}px, 
                            ${centerY*scale + pointY}px
                        )`;
                }
        ...
        const transformHandle = () => {
            ...
            return svg`
                <g 
                    ...
                    style="transform: translate(${centerX*this.panZoomFns.scale() + this.panZoomFns.x()}px, ${centerY*this.panZoomFns.scale() + this.panZoomFns.y()}px)"
                    >
                    <circle cx="0" cy="0" r="8" fill="black" class="hover:fill-orange-400 hover:opacity-100 opacity-60"/>
                    
                    <path d="M -8 11 L 0 22 L 8 11" fill="black" class="hover:fill-orange-400 hover:opacity-100 opacity-60"/>
                    
                    <path d="M 11 -8 L 22 0 L 11 8" fill="black" class="hover:fill-orange-400 hover:opacity-100 opacity-60"/>

                    <path d="M 30 0 C 30 18 18 30 0 30" stroke="black" stroke-width="4" stroke-linecap="round" fill="none" class="hover:stroke-orange-400 hover:opacity-100 opacity-60"/>

                    <g class="group">
                        <path d="M 30 0 L 40 0" stroke="black" stroke-width="4" stroke-linecap="round" fill="none" class="group-hover:stroke-orange-400 group-hover:opacity-100 opacity-60"/>
                        <circle cx="48" cy="0" r="7" stroke="black" stroke-width="3" stroke-linecap="round" fill="white" class="group-hover:stroke-orange-400 group-hover:opacity-100 opacity-60"></circle>
                    </g>

                    <g class="group">
                        <path d="M 0 30 L 0 40" stroke="black" stroke-width="4" stroke-linecap="round" fill="none" class="group-hover:stroke-orange-400 group-hover:opacity-100 opacity-60"/>
                        <circle cx="0" cy="48" r="7" stroke="black" stroke-width="3" stroke-linecap="round" fill="white" class="group-hover:stroke-orange-400 group-hover:opacity-100 opacity-60"></circle>
                    </g>

                    <g class="group">
                        <path d="M -10 -10 L -19 -19" stroke="black" stroke-width="4" stroke-linecap="round" fill="none" class="group-hover:stroke-orange-400 group-hover:opacity-100 opacity-60"/>
                        <circle cx="-25" cy="-25" r="7" stroke="black" stroke-width="3" stroke-linecap="round" fill="white" class="group-hover:stroke-orange-400 group-hover:opacity-100 opacity-60"></circle>
                    </g>
                </g>
            `
        }
            ...
            <div class="absolute w-3/4 h-3/4 inset-x-1/2 inset-y-1/2 translate-x-[-50%] translate-y-[-50%] z-[99999] rounded-xl bg-gray-200 flex overflow-hidden border border-black">
                ...
                <div class="relative grow h-full bg-gray-50">
                    <svg data-main-svg class="w-full h-full" transform="scale(1 -1)">
                        ...
                        <path data-fixed-scale d="M 0 -10 L 0 10 M -10 0 L 10 0" stroke-width="2" stroke="black" fill="none" vector-effect="non-scaling-stroke"/>
                        ${transformHandle()}
                    </svg>
                    ${transformPanel(this)}
                </div>
    ...
    return html`
        <div class="flex">
            <div class="flex justify-between items-center w-1/2">
                tx: 
                <input class="w-full text-center bg-gray-200 m-1" @change=${e => affineValueChange(e, "translateX")} .value=${translateX}/>
            </div>
            <div class="flex justify-between items-center w-1/2">
                ty: 
                <input class="w-full text-center bg-gray-200 m-1" @change=${e => affineValueChange(e, "translateY")} .value=${translateY}/>
            </div>
        </div>
        <div class="flex">
            <div class="flex justify-between items-center w-1/2">
                sx: 
                <input class="w-full text-center bg-gray-200 m-1" @change=${e => affineValueChange(e, "scaleX")} .value=${scaleX}/>
            </div>
            <div class="flex justify-between items-center w-1/2">
                sy: 
                <input class="w-full text-center bg-gray-200 m-1" @change=${e => affineValueChange(e, "scaleY")} .value=${scaleY}/>
            </div>
        </div>
        <div class="flex justify-between items-center w-1/2">
            rt: 
            <input class="w-full text-center bg-gray-200 m-1" @change=${e => affineValueChange(e, "rotate")} .value=${rotate}/>
        </div>
    `;
}
...
function transformPanel(el) {
    return html`
        <div class="absolute bottom-0 right-0 bg-gray-300 w-[165px] p-2 rounded">
            <div>
            ...
            <div class="${el.selectedGeometry.size === 0 ? "hidden" : ""} flex justify-evenly w-[150px] p-1">
